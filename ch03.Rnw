...
\ix{discrete frequency distributions}
...
\ix{dichotomous variables} \ix{variable!dichotomous}
...
\ix{polytomous variables} \ix{variable!polytomous}
have more outcome categories, which may be \emph{ordered} 
\ix{ordinal variables} \ix{variable!ordinal}
...
or \emph{unordered} (political party supported: Liberal, Conservative, Greens, Socialist). 
 \ix{nominal variables} \ix{variable!nominal}
...
\ix{binary variables} \ix{binomial samples}
...
\ix{Poisson samples} \ix{count data}
...
\ix{distributions} \ix{count data!distributions}
...
\ix{binomial distribution|(} \ixon{count data}
...
we could do the following (giving \figref{fig:dbinom12}): \ix{bar graph}
...
\ix{Poisson distribution|(} \ixon{count data}
...
An alternative test statistic is the likelihood-ratio $G^2$ \ix{likelihod-ratio test}
statistic,
...
It is useful to know that \func{goodfit} returns a list structure of \ix{list}
...
used for this purpose. \ix{Poisson distribution!visualization}
...
fit. \ix{Poisson distribution!visualization} \ix{binomial distribution!visualization}
...
It plots a quantity called a \IX{count metameter}
against \(k\), designed
...
the right panel of \figref{fig:distplot1}. \ix{leveled plot}
...
\input{ch03/tab/distparms} \ixon{binomial distribution}
...
First, we convert \data{Saxony} into a numeric data frame for use with \func{glm}. \ix{data frame}
<<sax-glm1>>=
data("Saxony", package = "vcd")
...
@
Finally, \figref{fig:sax-glm5} shows the rootogram for the double \ix{rootogram}
...
probability models to this one-way distribution. Rootograms for the simpler \ix{rootogram}
Poisson distribution and the negative binomial that allows for overdispersion are \ix{Poisson distribution}
\ix {binomial distribution}
