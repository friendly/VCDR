...
A useful brief introduction to this topic for binomial data
is the lecture notes by John Fox,
\url{http://socserv.socsci.mcmaster.ca/jfox/Courses/SPIDA/mle-mini-lecture-notes.pdf}.
}
...
<<dbinom2-plot2, echo=2:7, w=8, h=4, out.width=".75\\textwidth", cap="Binomial distributions for $k=0, \\dots, 12$ successes in $n=12$ trials, and four values of $p$">>=
...
\code{n} and \code{p}.
<<dnbin3, h=8, w=8, out.width=".8\\textwidth", cap="Negative binomial distributions for $n = 2, 4, 6$ and $p=0.2, 0.3, 0.4$, using xyplot">>=
