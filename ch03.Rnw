...
\input{front/vtoc03}   %% visual contents images
...
\ixon{discrete frequency distributions}
...
linear models (\chref{ch:glm}) where they serve as one component.  They also provide the basis for
a variety of recent extensions of regression models for count data (\chref{ch:glm}),
some of which account for the excess counts of zeros (zero-inflated models)
caused by left- or right-truncation often encountered in statistical practice.
...
The chapter then describes methods for fitting data to a distribution of a given form,
and presents simple but effective graphical methods that can be used to visualize goodness of fit,
to diagnose an appropriate model (e.g., does a given data set follow the
Poisson or negative binomial?) and to determine the impact of
individual observations on estimated parameters.
...
categories.  \textbf{\emph{Binary Events}} \ix{binary events} having two outcome categories include 
\ix{dichotomous variables} 
\ix{variable!dichotomous}
the toss of a coin (head/tails), sex of a child (male/female), survival of
a patient following surgery (lived/died), and so forth. \textbf{\emph{Polytomous Events}} \ix{polytomous events}
\ix{polytomous variables} 
\ix{variable!polytomous}
...
mechanism and provide the basis for testing scientific hypotheses and predicting
...
\ixon{binary variables} 
\ixon{binomial distribution}
\ixon{binomial samples}
\ixon{categorical data!dichotomous}
...
\ixon{binomial distribution!examples}
\begin{Example}[arbuthnot1]{Arbuthnot data}
Sex ratios, such as births of male to female children, have long been of interest
...
He calculated that, under the assumption
...
that any deviation from a constant sex ratio is relatively small, but also showed some systematic
trend over time.
...
\DONE{DM: use slightly simpler alternative? MF: rejected this because we're not explaining the code, but the code is
needed for \labref{lab:3.1}.}

% <<arbuthnot12, echo=-1, h=4, w=7, out.width=".75\\textwidth", cap="Arbuthnot's data on male/female sex ratios in London, 1629--1710, together with a (loess) smoothed curve over time and the mean Pr(Male)", scap="Arbuthnot's data on male/female sex ratios">>=
% data(Arbuthnot, package = "HistData")
...
\labref{lab:3.1} invites you to consider some other plots for this data.
...
Perhaps the most industrious dice-tosser of all time,
...
@
\end{Example}
\ixoff{binomial distribution!examples}

\ixoff{binary variables} 
\ixoff{binomial distribution}
\ixoff{binomial samples}
\ixoff{categorical data!dichotomous}

\subsection{Poisson data}\label{sec:pois-data}

\ixon{Poisson samples} 
\ixon{count data}
\ixon{categorical data!count}
...
\ixon{Poisson distribution!examples}
...
12 disputed papers.%
\footnote{
It should be noted that this is a landmark work in the development of statistical methods 
and their
application to the analysis of texts and
...
the works known by Hamilton (3 per 1000 words) and Madison (0.16 per 1000 words).
...
To answer these questions, they obtained data from the
...
\end{Example}
\ixoff{Poisson distribution!examples}
\ixoff{Poisson samples} 
\ixoff{categorical data!count}

\subsection{Type-token distributions}\label{sec:type-token}

\ixon{type-token}
\ixon{categorical data!type-token}

There are a variety of other types of discrete data distributions.
One important class is \textbf{\emph{type-token}} distributions, where
...
\end{Example}
\ixoff{type-token}
\ixoff{categorical data!type-token}

\section{Characteristics of discrete distributions}\label{sec:discrete-distrib}

\ixon{discrete frequency distributions!characteristics of}
...
for the probability that the variable $X$ takes the value $k$.
  \item[p] a cumulative probability/density function, or CDF,
  $F(k) = \sum_{X\le k} p(k)$.
...
the distribution functions available in the \basepkg{stats} package.

\input{ch03/tab/distfuns}

\ixoff{discrete frequency distributions!characteristics of}
...
number $k$ of events of interest that occur in $n$ independent trials
...
the number of redheads in randomly sampled groups of $n=10$
...
\ix{binomial distribution!moments of}
...
\footnote{
For the purpose of this book, we assume at least a basic familiarity with
the idea of maximum likelihood estimation.
A useful brief introduction to this topic for binomial data is
\citet[\S D.6]{Fox:2015}, available online.
}
...
\DONE{DM: either add ref to some text explaining Maximum Likelihood estimation,
...
\ixon{binomial distribution!calculation of}
...
Cumulative probabilities, summed to a vector of quantiles, \code{Q},
...
\ixoff{binomial distribution!calculation of}
...
we could do the following (giving \figref{fig:dbinom12}): 
\ix{bar graph}
% <<dbinom1, w=8, h=4, out.width=".75\\textwidth", cap="Binomial distribution for $k=0,\\dots,12$ successes in 12 trials and $p$=1/3.">>=
% k <- seq(0, 12)
% Pk <- dbinom(k, 12, 1/3)
...
% lines(x = k, y = Pk)
% @
%\DONE{DM: Why not directly using a barplot?}
...
% polygon.
We illustrate other styles for plotting in \secref{sec:poisson},
...
described later in \secref{sec:discrete-fit}, but a glance reveals that
...
% <<dbinom2-plot, w=8, h=4, out.width=".75\\textwidth", cap="Binomial distributions for $k=0, \\dots, 12$ successes in $n=12$ trials, and four values of $p$">>=
% library(lattice)
% mycol <- palette()[2:5]
...
% \DONE{DM: Avoid lattice to reduce complexity. Either only use ggplot2
%   throughout the chpater,
%   or use base graphics here, and ggplot2 for the remaining plots:}

\ixon{binomial distribution!plotting}
Finally, we can use programming features in \R to calculate and plot
probabilities for binomial distributions over a range of
both \code{k} and \code{p} as follows, for the purposes of
graphing the distributions as one or both varies.
The following code uses \func{outer} to create a $13 \times 4$ matrix \code{Prob}
containing the result of \func{dbinom} for
all combinations of \code{k = 0:12} and
\code{p = c(1/6, 1/3, 1/2, 2/3)}. These values are then supplied as
arguments to \func{dbinom}.
<<>>=
p <- c(1/6, 1/3, 1/2, 2/3)
k <- 0 : 12
...
str(Prob)
@
In this form, each column of \code{Prob} can be most easily plotted against \code{k}
using \func{matplot}. The following code generates \figref{fig:dbinom2-plot2}.
...
\ixoff{binomial samples}
\ixoff{categorical data!dichotomous}
\ixoff{binomial distribution!plotting}

\subsection{The Poisson distribution}\label{sec:poisson}

\ixon{Poisson distribution} 
\ixon{Poisson samples} 
\ixon{categorical data!count}
...
For the Poisson distribution, the probability function
is
...
\ix{Poisson distribution!moments of}
...
%\DONE{DM: Better add some book ref? Wikipedia is not persistent. MF: This is the permanent link.}
\footnote{
...
the number of goals scored by home teams and away teams follow
...
A quick check of whether the number of deaths among London cyclists
...
\ixon{Poisson distribution!calculation of}
...
of the values of \code{x} we wish to plot.
\ixoff{Poisson distribution!calculation of}

\begin{Example}[dpois-plot]{Plotting styles for discrete distributions}

\ixon{discrete frequency distributions!plotting}
\ixon{count data!plotting}
\ixon{Poisson distribution!plotting}
...
\figref{fig:dbinom2-plot2}.  With \func{xyplot}, the plot style is controlled
...
\ix{line graph}
The line-graph plot style of \figref{fig:dbinom2-plot2} has the advantage that
...
and enlarge the axis labels and titles (producing \figref{fig:dpois-ggplot2}).

\ix{graphical elements!legends}
\ix{graphical elements!axes}
...
\end{Example}
\ixoff{count data!plotting}
\ixoff{discrete frequency distributions!plotting}
\ixoff{Poisson distribution!plotting}
\ixoff{Poisson distribution} 
\ixoff{Poisson samples} 
\ixoff{categorical data!count}

\subsection{The negative binomial distribution}\label{sec:negbin}
\ixon{negative binomial distribution}
\ixon{categorical data!count}
...
\ixon{negative binomial distribution!moments of}
...
\ixoff{negative binomial distribution!moments of}
...
\ix{Poisson distribution!violated assumptions}
...
\begin{eqnarray}
\textrm{Mean}(X) = \mu = \frac{n (1-p)}{p} & \implies & p = \frac{n}{n+\mu} \comma \\
\textrm{Var}(X) = \frac{n (1-p)}{p^2} & \implies & \textrm{Var}(X) = \mu + \frac{\mu^2}{n} \period
\end{eqnarray}
...
\ixon{negative binomial distribution!calculation of}
...
\ixoff{negative binomial distribution!calculation of}
...
\ixoff{negative binomial distribution!plotting}
...
\ixoff{negative binomial distribution}
\ixoff{categorical data!count}

\subsection{The geometric distribution}\label{sec:geometric}

\ixon{geometric distribution}
\ixon{categorical data!count}
...
\ixon{geometric distribution!moments of}
\begin{eqnarray*}
\textrm{Mean}(X) & = & 1 / p\\
\textrm{Var}(X) &  = & (1-p) / p^2 \\
\textrm{Skew}(X) & = & (2-p) / \sqrt{1-p}
\end{eqnarray*}
\ixoff{geometric distribution!moments of}
...
\ix{geometric distribution!calculation of}
...
\ixoff{geometric distribution}
\ixoff{categorical data!count}

\subsection{The logarithmic series distribution}

\ixon{logarithmic series distribution}
\ixon{categorical data!count}
...
\ix{logarithmic series distribution!moments of}
For this distribution, the first two central moments are:
\begin{eqnarray*}
\textrm{Mean}(X) & = & \alpha \left(\frac{p}{1-p}\right) \comma\\
\textrm{Var}(X) &  = & -p \frac{p + \log(1-p)}{(1-p)^2 \log^2(1-p)} \period\\
\end{eqnarray*}
...
\ixoff{logarithmic series distribution}
\ixoff{count data}
\ixoff{categorical data!count}

\subsection{Power series family}\label{sec:pwrseries}
\ixon{power series distributions}
...
\ixoff{power series distributions}

\section{Fitting discrete distributions}\label{sec:discrete-fit}
\ixon{distributions!fitting}
\ixon{categorical data!fitting}
...
\ix{statistical analyses!goodness-of-fit} 
...
\ix{statistical analyses!likelihood ratio test}
statistic,
...
with the \data{HorseKicks} data, where we expect a Poisson
...
(green/yellow) in a second generation were uncomfortably too
...
In \R, the function \func{fitdistr} in the \Rpackage{MASS}
...
If the parameters are not specified they are estimated either by maximum likelihood (ML) or Minimum Chi-squared. \func{print} and \func{summary} methods for
...
\ix{list}
named components that are used by method functions for class \class{goodfit}
...
grouped as ``10+.''
In this case, it is necessary to supply the correct
...
\ixoff{Poisson distribution!examples}
\ixon{negative binomial distribution!examples}
<<Fedfit2>>=
summary(Fed_fit0)
@
...
summary(Fed_fit1)
@
Recall that the Poisson distribution assumes that the probability of a word like \emph{may}
...
\ixoff{negative binomial distribution!examples}
\end{Example}
...
\ix{quantile--quantile plots}
\footnote{
Quantile--quantile (QQ) plots are a common alternative
for the goal of comparing observed and expected values
under some distribution.  These plots are useful for
unstructured samples, but less so when we also want to
...
\ix{Poisson distribution!plotting}
\ix{spline}
...
suggests plotting the frequencies  on a square-root scale,
...
% \DONE{PC: Couldn't you go one stem further, and plot a colour aesthetic for the standardized
% residuals for each hanging block? It's great that the (O - E) effect is captured for detecting,
% systematic effects, but one would probably be interested in the $\sqrt{(O-E)^2 / E}$ effect too in
% visual inspections. Would really only make sense for the devation rootogram plot}

All of these plots are actually produced by the \func{rootogram} function
in \pkg{vcd}, the \func{plot} method for \class{goodfit} objects.  The default is \code{type = "hanging"}, and there are many
options to control the plot details.  For example, the plots in \figref{fig:Fed0-plots2}
use \code{shade=TRUE} to color the bars according to the contribution to the
Pearson chi-square.%
\footnote{
The bipolar color scheme uses \textcolor{blue}{blue} for positive standard Pearson residuals, $(n_k - N \hat{p}_k) / \sqrt{N \hat{p}_k}$,
and \textcolor{red}{red} for negative residuals,
with shading intensity proportional to the categorized absolute values shown in the legend.
}
...
fit. 
\ix{Poisson distribution!plotting} 
\ix{binomial distribution!plotting}
...
@
Comparing the two plots in \figref{fig:Fed0-Fed1}, we can see that the Poisson model overestimates the frequency of counts $k=1$ and underestimates the larger counts for $k=$ 4--6 occurrences. The surprising feature here is that the greatest contribution
to lack of fit for the Poisson model is the frequency for $k=6$.
...
\ixoff{distributions!fitting}
\ixoff{categorical data!fitting}

\section{Diagnosing discrete distributions: Ord plots}\label{sec:discrete-ord}

\ixon{Ord plot}
\ixon{distributions!diagnosing}
\ixon{categorical frequency distributions!diagnosing}
...
ord
@
In this plot, the black line shows the usual ordinary least squares (OLS)
regression fit of frequency, $n_k$, on number of occurrences, $k$;  the red line shows
the weighted least squares fit, using weights of $\sqrt{n_k-1}$.
In this case, the two lines are fairly close together, in regards to
...
favorably with the maximum likelihood estimate (MLE), $\lambda=0.610$ and the
...
\figref{fig:ordplot3plot} (left) shows the Ord plot for the \data{Federalist} data.
...
\citet{JinkinsonSlater:81} and \citet{HoaglinTukey:85}
...
Ord_plot(WomenQueue, main = "Women in queues of length 10",
         gp = gpar(cex = 1), pch = 16)
@
\figref{fig:ordplot3plot} (right) shows the Ord plot for these data.
...
\begin{itemize*}
\item The Ord plot lacks resistance, because a single discrepant
...
\ixoff{Ord plot!limitations}
\ixoff{Ord plot}
\ixoff{distributions!diagnosing}
\ixoff{categorical frequency distributions!diagnosing}


\section{Poissonness plots and generalized distribution plots}\label{sec:discrete-Poissonness}

\ixon{Poissonness plot}
\ixon{Poisson distribution!plotting}

The \textbf{\emph{Poissonness plot}}
\citep{Hoaglin:80}
is a robust plot to sensitively determine how well a
one-way table of frequencies follows a \IX{Poisson distribution}.
It plots a quantity called a \ixmain{count metameter}
against \(k\), designed
...
developed similar plots for other discrete distributions,
...
of these plots for the Poisson distribution; then (\secref{sec:discrete-other})
...
\item \boldital{Influence}:  Extensions of the method result in
       plots that show the effect of each point on the estimate of
...
and $\hat{\lambda} = e^b$ should be reasonably close in value
...
% \DONE{PC: Wouldn't a bootstrap estimate be just as effective here? And given that it's count data,
% it would cost very little extra resources to obtain. Perhaps a footnote about this metioning
% the bootstrap method as another viable option}
...
\begin{figure}[tb]
\centerline{
\includegraphics[width=.49\textwidth]{ch03/fig/distplot1-1}
\includegraphics[width=.49\textwidth]{ch03/fig/distplot2-1}
}
...
% \DONE{DM: In the leveled plot, the label for the slope is actually
...
For this family, \citet{HoaglinTukey:85} developed similar plots
...
% \DONE{DM: The following section assumes knowledge of GLMs, introduced in a
% later chapter. Either remove chapter, or move to the end after the GLM
% chapter --- Made a `hard' section }

\section[Fitting discrete distributions as generalized linear models\hard]{Fitting discrete distributions as generalized linear \linebreak models\hard}\label{sec:fitglm}

\ixon{generalized linear models}
...
Another general family of distributions---the \term{exponential family}---includes
most of the common continuous distributions:
...
\ix{distributions!double binomial}
...
and the offset term is a value that does not depend on the
parameters.
...
\textsf{rmutil} package, providing the standard complement of
...
This package is not on CRAN, but is available at \url{http://www.commanster.eu/rcode.html}.
}
...
\ix{rootogram}
binomial, which can be compared with the binomial model
...
% something wrong here; converted to eval=FALSE
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{ch03/fig/sax-glm5-1}
\caption{Rootogram for the double binomial model for the Saxony data. This now fits well in the tails of the distribution.}
\label{fig:sax-glm5}
\end{figure}
...
\end{Example}
\ixoff{generalized linear models}
...
\ixoff{distributions} 
\ixoff{count data}
\ixoff{count data!distributions}
\ixoff{discrete frequency distributions}
...
<<cleanup03, include=FALSE, purl=FALSE>>=
