...
\exercise How many years does it take to get into the baseball Hall of Fame?
...
\end{tabular}
    ...
    \begin{enumerate*}
      \item For the Poisson distribution, show that the probability function can be expressed in the
      form
\begin{equation*}
\Pr \{ X = k \given k>0\} )=
