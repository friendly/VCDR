...
\ix{categorical data!definition}
...
\ix{categorical data!statistical methods}
...
\term{binary variables} 
\ix{variable!binary} (or \term{dichotomous variables}) 
\ix{variable!dichotomous} such as \var{Gender}
...
may have ordered categories (and are called \term{ordinal variables}), 
\ix{variable!ordinal}
while others (\term{nominal variables}) like \var{Marital status} 
\ix{variable!nominal}
...
\ix{graphical elements!axis labels} 
\ix{axes}
...
\ix{graphical elements!attributes}
...
\ix{graphical elements!annotations}
