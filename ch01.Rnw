...
``Data visualization'' \ixmain{data visualization} can mean many things, from popular press
...
``Categorical'' \ix{categorical data!definition} means different things in different
...
Statistical methods for the analysis of categorical data \ix{categorical data!statistical methods} also fall into two
...
\term{binary variables} \ix{variable!binary} (or \term{dichotomous variables}) \ix{variable!dichotomous} such as \var{Gender}
from those with more than two categories (called \term{polytomous variables}). \ix{variable!polytomous}
...
may have ordered categories (and are called \term{ordinal variables}), \ix{variable!ordinal}
while others (\term{nominal variables}) like \var{Marital status} \ix{variable!nominal}
...
or data in \term{case form}. \ix{data|case form}
...
Data in \term{frequency form} \ix{data|frequency form}
...
comes from a different person. \ix{data|frequency vs count}
...
analysis. \ix{variable!number of}
...
variables not represented in the table. \ix{contingency table}
For data in case form, this means simply ignoring (or not recording)
one or more variables;  the ``observations'' remain the same. \ix{data|case form}
Data in frequency form, however, result in smaller tables when
any variable is ignored;  the ``observations'' are the cells of
the \ctab. \ix{data|frequency form} For example, in the \data{Arthritis} data, ignoring \var{Sex}
...
\ix{variable!response \~|(} \ix{variable!explanatory \~|(}
Most statistical models make a distinction between \term{response variables}
(or \emph{dependent}, or \emph{criterion} variables)
\index{dependent variables|see {response variables}}
\index{criterion variables|see {response variables}}
and
\term{explanatory variables}
(or \emph{independent}, or \emph{predictor} variables). 
\index{independent variables|see {explanatory variables}}
\index{predictor variables|see {explanatory variables}}
...
to be explained by the other, explanatory variables. \ix{analysis of variance}
...
\ix{data analysis!hypothesis testing approach \~|(} \ix{hypothesis testing \~|(}
...
indicating substantial departure from independence.  \ix{statistical analyses!Cramer's V} \ix{Cramer's V}
...
\ix{data analysis!hypothesis testing \~|)} \ix{hypothesis testing \~|)}

\subsection{Model building approaches}
\ix{data analysis!model building \~|(} \ix{model building \~|(}
...
Logistic regression uses the \term{logit function}, \ix{statistical analyses!logistic regression} \ix{logistic regression}
\begin{equation*}
...
\ixon{statistical analyses!logistic regression} \ixon{logistic regression}
...
\ixoff{statistical analyses!logistic regression} \ixoff{logistic regression}
\ix{data analysis!model building \~|)} \ix{model building \~|)}
...
\end{figure}
\figref{fig:datadisp} \ix{graphics!basic functions}
...
\ixon{graphics!analysis graphic}
...
a graph designed for \emph{analysis}---a mosaic display (\chref{ch:mosaic}) \ix{mosaic display}
...
\caption{Analysis graph: A mosaic display showing the relationship between skin color and release type for those arrested on a charge of simple possession of marijuana in Toronto, 1996--2002.}\label{fig:arrests0-mosaic}
\end{figure}
...
\ixoff{graphics!analysis graphic}

\ixon{graphics!presentation graphic}
...
\ixoff{graphics!presentation graphic}
...
\ixon{effect ordering} \ixon{variable!effect ordering}
...
and detection of patterns, trends, and anomalies. \ix{variable!discrete}
...
\ixoff{effect ordering} \ixoff{variable!effect ordering}
...
The precept here is that \ix{graphics!design principals} \ix{visualization}
...
NASA staff analyzed the data, tables, and charts submitted by
...
\ix{graphics!elements!axis labels} \ix{axes}
...
\ix{graphics!elements!attributes}
...
\ix{graphics!elements!annotations}
...
\item[colors] Whereas a good choice of colors can greatly enhance a
  ...
  (appropriate for unordered categories) \ix{color palettes} 
  taken from two different color spaces: Hue-Saturation-Value (HSV) \ix{color palettes!Hue-Saturation-Value}
  and Hue-Chroma-Luminance (HCL), where only the hue is varied. \ix{color palettes!Hue-Chroma-Luminance}
