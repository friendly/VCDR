...
\ixon{loglinear models!extensions}
...
than is available from models for general association.
...
\ix{loglinear models!multivariate responses}
...
\ixon{ordinal variables!three-way tables}
<<ord3way, child='ch10/ord3way.Rnw'>>=
@
\ixoff{three-way table}
\ixoff{multi-way table}
\ixoff{ordinal variables!three-way tables}
...
\section{Multivariate responses\hard}\label{sec:loglin-multiv}
...
\ixoff{loglinear models!extensions}
...
<<exercises10, child="ch10/exercises.Rnw">>=
@

% Cleanup local variables
<<cleanup10, size='footnotesize', include=FALSE>>=
