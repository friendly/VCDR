# From: http://leonawicz.github.io/ProjectManagement/func_convert.html

convertOne <- function(file, outDir=".", 
                       rmdChunkID = c("```{r", "}",   "```"), 
                       rnwChunkID = c("<<",    ">>=", "@"), 
                       emphasis = "replace", overwrite = TRUE, ...) {

    ext <- tail(strsplit(file, "\\.")[[1]], 1)
    if (ext == "Rmd") {
        .swap(file, header = NULL, outDir = outDir, rmdChunkID = rmdChunkID, 
...
#convertDocs converts between Rmd and Rnw files. The project?s docs/Rmd or
#docs/Rnw directory is specified. Any files of the same type as the directory are
#converted to the other type and saved to the other directory. The input files
#are not removed.
#
#This function speeds up the process of duplicating files, e.g., when wanting to
#make pdf files from Rnw files when only Rmd files exist. This is almost
#exclusively what I use this function for. On less frequent occasions I have used
#it in the other direction when I have Rnw files which were once used to make
#PDFs but later I decide to put them on the web as a web page and not as a link
#to a pdf files.
#
#The user still makes specific changes by hand, for example, any required changes
#to knitr code chunk options that must differ for pdf output vs. html output. The
#primary benefit is in not having to fuss with large amounts of standard
#substitutions which can be automated, such as swapping code chunk enclosure
#styles and common file metadata. Of course, this function is not perfect. It
#amounts to a text-parsing hack that is intended to handle the most common of
#idiosyncrasies and differences which exist between my own Rmd and Rnw files in
#the context of my own limited rule set.

# Rmd <-> Rnw document conversion Main conversion function
convertDocs <- function(path, 
                        rmdChunkID = c("```{r", "}", "```"), 
                        rnwChunkID = c("<<", ">>=", "@"), 
...
#  It swaps section headings formatting. It is called directly by .swap, internal to convertDocs.
# Rmd <-> Rnw document conversion Conversion support functions called by
# .swap()
