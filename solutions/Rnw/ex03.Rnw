...
\section{Chapter 3: Fitting and Graphing Discrete Distributions}
...
(a) Make a plot of \texttt{Ratio} over \texttt{Year}, similar to Figure 3.1. What features stand out? 
...
(b) Plot the total number of christenings, \texttt{Males + Females} or \texttt{Total} (in 000s) over time.
...
(a) With \texttt{xyplot()}, try the different plot formats using points connected with lines, as in Figure 3., or using points and lines down to the origin, as in the panels of Figure 3.10.
...
(b) Also with \texttt{xyplot()}, produce one version of a multi-line plot in a single panel that you think shows well how these distributions change with the probability $p$ of success.
...
Use the data set \texttt{WomenQueue} to:
...
(b) Check for goodness-of-fit to the binomial distribution using the \texttt{goodfit()} methods described
in Section 3.3.2.
...
\section{The ratio of Chi-square/df}
ss[,"X^2"] / ss[,"df"]
@
...
(c) Use the \texttt{plot.goodfit()} method to visualize these two models.
...
For the \texttt{Federalist} data, the examples in Section 3.3.1 and Section 3.3.2 showed the negative binomial to provide an acceptable fit. Compare this with the simpler special case of geometric distribution, corresponding to $n = 1$.

(a) Use \texttt{goodfit()} to fit the geometric distribution. [Hint: use \texttt{type="nbinomial"}, but specify \texttt{size=1} as a parameter.]
...
fd.df
fd.nbin <- glm.nb(wordc ~ block, method = "glm.fit", data = fd.df)     #NB:  Does not converge
...
column, suitable for use with \texttt{goodfit()}.
...
(b) Following Example 3.13, use \texttt{goodfit()} to fit the binomial model and plot the results. Is there an indication that the binomial does not fit these data? 
<< ex3_7b>>=
...
(d) Try fitting the negative binomial distribution, and use \texttt{distplot()} to diagnose whether the negative binomial is a reasonable fit. 
<< ex3_7d>>=
...
The data frame \texttt{Bundesliga} gives a similar data set to that for UK soccer scores (UKSoccer) examined in Example 3.9, but over a wide range of years. The following lines calculate a two-way table, \texttt{BL1995}, of home-team and away-team goals for the 306 games in the year 1995.
...
(a) As in Example 3.9, find the one-way distributions of \texttt{HomeGoals}, \texttt{AwayGoals}, and \texttt{TotalGoals = HomeGoals + AwayGoals}.
<< 3x3.8a>>=
...
(b) Use \texttt{goodfit()} to fit and plot the Poisson distribution to each of these. Does the Poisson seem to provide a reasonable fit? 

(c) Use \texttt{distplot()} to assess fit of the Poisson distribution. 
...
Repeat the exercise above, this time using the data for all years in which there was the standard number (306) of games, that is for \texttt{Year>1965}, tabulated as shown below. 
<< BL>>=
...
Using the data \texttt{CyclingDeaths} introduced in Example 3.6 and the one-way frequency table 
\texttt{CyclingDeaths.tab <- table(CyclingDeaths$deaths)}, 

(a) Make a sensible plot of the number of deaths over time. For extra credit, add a smoothed curve (e.g., using \texttt{lines(lowess(...))}).
<< ex3_10a>>=
...
(b) Test the goodness of fit of the table \texttt{CyclingDeaths.tab} to a Poisson distribution statistically using \texttt{goodfit()}. 

(c) Continue this analysis using a \texttt{rootogram()} and \texttt{distplot()}. 
...
The one-way table, \texttt{Depends}, in vcdExtra [not] shown below gives the frequency distribution of the number of dependencies declared in 
...
(b) Use \texttt{Ord\_plot()} to see if this method can diagnose the form of the distribution. 
...
How many years does it take to get into the baseball Hall of Fame? The Lahman (Friendly, 2014b) package provides a complete record of historical baseball statistics from 1871 to the present. One table, \texttt{HallOfFame}, records the history of players nominated to the Baseball Hall of Fame, and those eventually inducted. 

The table [not shown] below, calculated in \texttt{help(HallOfFame, package="Lahman")}, records the distribution of the number of years taken (from first nomination) for the 109 players in the Hall of Fame to be inducted (1936--present). Note that \texttt{years==0} does not, and cannot, occur in this table, so the distribution is restricted to positive counts. 

Such distributions are called \textbf{\_zero-truncated distributions\_}. 
...
(c) Enter these data into R as a one-way table, and use \texttt{goodfit()} to fit the standard Poisson distribution, as if you hadn't encountered the problem of zero truncation. 
