...
@
\section{Chapter 6: Correspondence Analysis}

These solutions use an updated version of the \texttt{ca} package, v. 0.64 or greater.

\subsection{Exercise 6.1 }

The \texttt{JobSat} data in vcdExtra gives a $4 \times 4$ table recording job satisfaction in relation to income. 
...
Refer to Exercise 5.1 in Chapter 5. Carry out a simple correspondence analysis on the $4 \times 5$ table \texttt{criminal} from the logmult package. 
...
Refer to Exercise 5.2 for a description of the \texttt{AirCrash} data from the vcdExtra package. Carry out a simple correspondence analysis on the $5 \times 5$ table of \texttt{Phase} of the flight and \texttt{Cause} of the crash. 
...
(c) The default plot method uses \texttt{map="symmetric"} with points for both rows and columns. Try using \texttt{map="symbiplot"} with vectors (\texttt{arrows=}) for either rows or columns. (Read \texttt{help(plot.ca)} for a description of these options.) 
<< ex6_3c>>=
...
The data set \texttt{caith} in MASS gives a classic table tabulating hair color and eye color of people in Caithness, Scotland, originally from Fisher (1940). 
...
The same data, plus a similar table for Aberdeen, are given as a three-way table as \texttt{HairEyePlace} in vcdExtra. 
...
The data set \texttt{Gilby} in vcdExtra gives a classic (but now politically incorrect) $6 \times 4$ table of English schoolboys classified according to their clothing and their teacher's rating of "dullness"" (lack of intelligence). 
...
<< ex 6_6b, fig.height=4, fig.width=6>>=
library(vcd)
...
@
The association between \texttt{ses} and \texttt{mental} in Figure 6.2 appears to be nearly entirely 
...
and highlights one cell for \texttt{ses=="2"} and \texttt{mental=="impaired"} that deviates from this 
pattern.
...
(a) Re-do this example, but in the call to \texttt{rpois()}, specify a non-negative vector of Poisson means to 
...
\section{reduce number of levels of Time }
...
(a) Use the stacking approach (Section 6.3) to perform a CA of the table with \texttt{Network} and \texttt{Time} coded interactively. You can create this using the \texttt{as.matrix()} method for a "structable" object. 
<< >>=
...
(d) Carry out an MCA analysis using \texttt{mjca()} of the three-way table \texttt{TV3}. Plot the 2D solution, and compare this with both the CA plot and the solution for the stacked three-way table. 
<< ex6_9d>>=
TV3.mca <- mjca(TV3)
plot(TV3.mca)
@
...
Refer to Exercise 5.10 for a description of the \texttt{Vietnam} data set in vcdExtra. 
...
(c) Use \texttt{mjca()} to carry out an MCA on the three-way table. Make a useful plot of the solution and interpret in terms of the relationship of the response to year and sex. 


\subsection{Exercise 6.12 }

Refer to Exercise 5.9 for a description of the \texttt{Accident} data set in vcdExtra. The data set is in the form of a frequency data frame, so first convert to table form. 
<<>>=
data("Accident", package="vcdExtra")
accident.tab <-xtabs(Freq ~ age + result + mode + gender, data=Accident)
@

(a) Use \texttt{mjca()} to carry out an MCA on the four-way table \texttt{accident.tab}. 
<< ex6_12a>>=
...
(a) Use \texttt{mjca()} to carry out an MCA on the three-way table UCBAdmissions. 
