...
@
\section{Chapter 5: Mosaic Displays for n-Way Tables}

\subsection{Exercise 5.1}

The data set \texttt{criminal} in the package logmult gives
...
(a) Use \texttt{loglm()} to test whether there is an association between Year and Age.
Is there evidence that dropping of charges in relation to age changed
over the years recorded here?

b.Use \texttt{mosaic()} with the option \texttt{shade=TRUE} to display the pattern of signs
and magnitudes of the residuals. Compare this with the result of \texttt{mosaic()}
using “Friendly shading,” from the option \texttt{gp=shading\_Friendly}. Describe
...
<!-- [lab:mosaic-crash] -->
The data set \texttt{AirCrash} in vcdExtra gives a database of
...
Start by making a frequency table, \texttt{aircrash.tab`: 
<< aircrash-tab>>=
data("AirCrash", package = "vcdExtra") 
aircrash.tab <- xtabs(~ Phase + Cause, data = AirCrash) 
@

(a) Make a default mosaic display of the data with \texttt{shade=TRUE} and interpret
...
with some of the \texttt{labeling\_args} options (\texttt{abbreviate}, \texttt{rot\_labels}, etc.)
to see if you can make the plot more readable. *Hint*: a variety of
these are illustrated in §4.1 of \texttt{vignette("strucplot")}.
...
League Baseball from 1871 through 2012. For all players, the \texttt{Master}
...
three-way association, $H\_4$ in 5.2.
...
are a strawberry. The data set \texttt{jansen.strawberry} from the agridat gives
...
(a) Use \texttt{pairs(jansen.tab, shade=TRUE)} to display the pairwise associations
...
association is MF,C, or \texttt{~ 1*2 + 3}. Fit this model using \texttt{loglm()} and
display the residuals from this model with mosaic. Describe the pattern
of lack of fit of this model.

\subsection{Exercise 5.7}

The data set \texttt{caith} in MASS gives another classic $4 \times 5$ table
...
use \texttt{Caith <- as.matrix(caith)} to convert to array form. Examine the
result, and use \texttt{names(dimnames(Caith))<-c()} to assign appropriate names
to the row and column dimensions.

(b) Fit the model of independence to the resulting matrix using \texttt{loglm()}.
...
The \texttt{HairEyePlace} data in vcdExtra gives similar data on hair color and
...
\texttt{~ Hair + Eye | Place} and both mosaic and cotabplot. It is probably
...
<!-- [lab:mosaic-accident]-->
Bertin (1983, pp. 30-31) used a 4-way table
...
\texttt{Freq ~ age+mode+gender+result} to this data set.
...
the model \texttt{Freq ~ age*mode*gender + result} that asserts independence of
...
<!-- [lab:mosaic-vietnam] -->
The data set Vietnam in vcdExtra gives a
...
(b) Use the functions shown in the table together with \texttt{loglin2formula()} to
print the corresponding model formulas for each.

\subsection{Exercise 5.12}

<!-- [lab:mosaic-titanic]  -->
The dataset \texttt{Titanic} classifies the 2,201 pasengers
