...
the UK Football Association. <<UKsoccer-ex\>\>= data("UKSoccer", package
= "vcd") ftable(UKSoccer) @ This two-way table classifies all
...
Find the marginal total of the number of goals scored by each of the
home and away teams.

Express each of the marginal totals as proportions.

Comment on the distribution of the numbers of home-team and away-team
...
with 12 children. This data set is used extensively in ch:discrete.
<<\>\>= data("Saxony", package = "vcd") Saxony @ Another data set,
...
Saxony12 from the Geissler data. <<\>\>= data("Geissler", package =
"vcdExtra") str(Geissler) @

Use subset to create a data frame, sax12 containing the Geissler
...
Do the same steps again to create a one-way table, Saxony11, containing
...
Try this the long way: convert UCBAdmissions to a data frame
(as.data.frame), manipulate the factors (e.g., interaction), then
convert back to a table (as.data.frame).

Try this the short way: both ftable and structable have as.matrix
methods that convert their result to a matrix.

The data set VisualAcuity in vcd gives a $4 \times 4 \times 2$ table as
a frequency data frame. <<VA-df\>\>= data("VisualAcuity", package =
"vcd") str(VisualAcuity) @

From this, use xtabs to create two $4\times 4$ frequency tables, one for
each gender.
