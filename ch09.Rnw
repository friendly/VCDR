...
\input{front/vtoc09}   %% visual contents images
...
\ix{correlation}
\ix{logistic regression!multivariate}
\ix{regression!multivariate}
\ix{generalized linear model!multivariate}
In this sense,
...
In \secref{sec:loglin-zeros} we consider problems that arise in
sparce contingency tables containing cells with frequencies of zero.
...
\ix{loglinear models!independence model}
for a two-way table.
...
\ix{contrasts!treatment}
\ix{contrasts!sum-to-zero}
...
Thus, the saturated model in \eqref{eq:lsat} contains $\lambda_{ij}^{AB}$,
...
notation that lists only the high-order terms: model \eqref{eq:lsat}
is denoted $[A B]$, while model \eqref{eq:lmain} is $[A] [B]$.
\ixoff{analysis of variance}
...
of various types of independence relations, as illustrated in \tabref{tab:hyp3way}.
...
\ix{saturated model}
\ix{constraints!zero-sum}
\ix{zero-sum constraints}
For example, for
...
\ixoff{loglinear models!as GLMs}
...
Terms that are excluded from the model go
...
@
\noindent The variables are represented by their margin index; margins
combined in the same vector represent an interaction term.
...
@
\noindent (Note that the formula expression expands to \code{A*A + A*B
+ A*C + B*A + B*B + B*C + C*A + C*B + C*C}. Since terms like \code{A*A} become
\code{A} and duplicate terms are ignored, this eventually yields \code{A + B + C
  + A*B + A*C + B*C}---all second-order terms and the corresponding
main effects.)
...
Some special considerations when there are cells with
...
\ixon{loglinear models!goodness-of-fit}

\ix{multi-way table}
\ix{goodness-of-fit!loglinear models}
...
\textbf{\emph{nested models}} by their difference in \GSQ statistics,
...
than the more general model $M_1$, i.e., $\GSQ (M_2) \ge \GSQ (M_1)$.
...
as
\begin{equation*}
\mbox{AIC} = -2 \log \mathcal{L} + 2 k \comma
\end{equation*}
...
\ixoff{loglinear models!goodness-of-fit}

\subsection{Residuals for \loglin models}\label{sec:loglin-residuals}
\ixon{loglinear models!residuals}
\ixon{residuals!loglinear models}
...
offers important clues regarding the nature of lack of fit and can suggest
...
r_i = \frac{n_i - \widehat{m}_i}{\sqrt{\widehat{m}_i}} \period
\end{equation}
Similarly, the \term{deviance residual} can be defined as
\begin{equation}\label{eq:resdev2}
g_i = \sign({n_i - \widehat{m}_i}) \sqrt{2 n_i \log (n_i / \widehat{m}_i) - 2 (n_i - \widehat{m}_i) } \period
\end{equation}
...
\begin{eqnarray*}
r_i^\star & = & r_i / \sqrt{1 - h_i} \comma \\
g_i^\star & = & g_i / \sqrt{1 - h_i} \period
\end{eqnarray*}
...
fitted values, $\widehat{m}_i$, in the model for the
...
\code{rstandard(model, type=)}, for \code{type="pearson"} and \code{type=} \code{"deviance"}
versions.
\ixoff{loglinear models!residuals}
\ixoff{residuals!loglinear models}
...
$\LLM{AD, GD}$, which allows associations of admission with department and
...
\ixon{loglinear models!plotting}
The \func{mosaic} method for \class{glm} objects in \pkg{vcdExtra}
provides a \code{residuals\_type} argument, allowing
\code{residuals\_type = "rstandard"}
...
This suggests a model that allows an association between Admission and Gender in Department
A only,
...
\ixoff{loglinear models!plotting}
...
\begin{eqnarray*}
  ...
  \period
\end{eqnarray*}
Substituting from \eqref{eq:lno3way}, all terms that do not
...
correspond directly to $[AC]$ and $[BC]$ in the \loglin\ model \eqref{eq:lno3way}. 
The association among the explanatory variables,
...
Conversely, any \loglin model that includes all associations among
...
We illustrate the features of the logit model that lead to the same
...
\ixon{logit models!interpretation}
\ix{reference category}
\ix{baseline models}
...
Anova(berk.logit3)
@
\ixoff{logit models!interpretation}

\subsubsection*{Plotting logit models}

\ixon{logit models!plotting}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.49\textwidth]{ch09/fig/berk-logit2}
\includegraphics[width=0.49\textwidth]{ch09/fig/berk-logit3}
...
\end{Example}
\ixoff{logit models}
\ixoff{logit models!plotting}

\section{Zero frequencies}\label{sec:loglin-zeros}
\ixon{zero frequencies}
...
\ix{zero frequencies!structural zeros}
...
\ix{zero frequencies!sampling zeros}
...
alternatively, zero frequencies can be set to \code{NA}.
...
\begin{equation*}
df = \mbox{\# of cells} - \mbox{\# of fitted parameters} \period
\end{equation*}
...
df = (\mbox{\# cells} - \mbox{\# of parameters})
   - (\mbox{\# zero cells} - \mbox{\# of NA parameters}) \comma
\end{equation}
...
data for unsaturated models, and should be deprecated, although it is widely
used in practice.
...
among males are clearly structural zeros and therefore one
structural zero in the concern-by-gender marginal table.
As usual, we abbreviate the table variables concern, age, and gender
by their initial letters, C, A, G below.

\input{ch09/tab/health}
...
marking them with a small ``0,'' as these have an area of zero.
...
\code{start} argument. 
%\TODO{Check whether this will work with LRstats()}
<<health-loglm2>>=
...
@
% \TODO{Test bug fix with LRstats()}
% <<health-loglm2-test>>=
% LRstats(health.loglm0, health.loglm1, health.loglm2)
% @
...
% 
% <<multiv, child='ch09/multiv.Rnw'>>=
% @
% 
% <<coalminers-ex, child='ch09/coalminers.Rnw'>>=
% @
% 
...
% example.
% 
% <<tox-ex, child='ch09/toxaemia.Rnw'>>=
% @
...
<<cleanup09, include=FALSE, purl=FALSE>>=
