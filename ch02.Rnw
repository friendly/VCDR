...
\TODO{DM: The chapter does not cover NA handling, although this is needed
  in some chapters. Maybe treat this in the also missing section on subsetting?}
...
in the same way than a matrix. Additionally, variables can be extracted
...
\TODO{DM: Add this to vcd or vcdExtra.}
...
\noindent Finally, we note that data frames can conveniently be edited using,
e.g., \code{fix(Arthritis)}.
...
\TODO{DM: also show HTML version? -- I think this is unnecessary}
...
tv_data <- file.choose()
@
\noindent to select the path using the file chooser tool.
...
Here, we use \func{collapse.table}, which was designed for this purpose.
