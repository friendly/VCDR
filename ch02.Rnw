...
\epigraph{I'm a tidy sort of bloke. I don't like chaos. I kept records in the record rack, tea in the tea caddy, and pot in the pot box}{George Harrison, from \url{http://www.brainyquote.com/quotes/keywords/tidy.html}}
...
to represent categorical data. More importantly, you also need
to be able to convert these from one
...
In \R, a \textbf{\emph{vector}} is a collection of values, like numbers,
character strings, or logicals (\code{TRUE, FALSE}), and often correspond to a variable in some analysis.
\textbf{\emph{Matrices}}
\ix{matrices}
are rectangular arrays like a traditional table, composed of vectors in their columns
or rows.
\textbf{\emph{Arrays}}
\ix{arrays}
add additional dimensions, so that, for example, a 3-way table can be represented
...
data structure is a \emph{list}, a generic vector that can contain 
\ix{list}
...
in the same way as a matrix, using subscripting (\code{[,]}). Additionally, variables can be extracted
...
@ 
\noindent Values in data frames can also be edited conveniently using,
e.g., \code{fix(mydata)}, opening a simple, spreadsheet-like editor.

\ixon{data frame!importing}
\ixon{importing data}
...
than the number of columns;
  ...
  \item[\code{na.strings}] (default: \code{"NA"}) refers to one or more strings that are interpreted
...
respectively, and
\code{header=TRUE}.
\ixoff{data frame!importing}
\ixoff{importing data}
\ixoff{data frame}
...
as \code{c("None", "Some", "Marked")}.  We can correct this here by
...
factors, and the \emph{positions} of the values in the rows and columns reflect their
...
\ixoff{table form!converting to}
...
distinguish explicitly between subsetting levels and subsetting
...
hec[[c("Male", "Brown"),]]
@ 
\ixoff{structable objects!subsetting}
\ixoff{data!table form}
\ixoff{table form}

\subsection[Subsetting data frames]{Subsetting data frames}\label{sec:subsettingdf}

\ixon{data frame!subsetting}
...
we want to analyze, or else, having done some initial analyses, we decide that
...
the factors that are not shown in the plots.
...
\ix{data frame!converting} 
\ix{case form!converting} 
\ix{frequency form!converting} 
\ix{table form!converting}
\begin{table}[htb]
...
\footnote{
Because \R is object-oriented, this is actually a shorthand for
...
See \citet{Leifeld:2013:JSS} for a description of the \Rpackage{texreg}
...
%% removed results='asis' because it prints the chunk in RGB
<<xtable3, eval=FALSE>>=
xtable(HorseKicks)
@
\begin{table}[ht]
...
The code below uses the following techniques, giving the version shown in \tabref{tab:xtable5}:
\begin{seriate}
\item \func{addmargins} is used to show the sum of all the frequency values;
\item \func{t} transposes the data frame to have 2 rows;
\item \func{rownames} assigns the labels we want for the rows;
\item using the \code{caption} argument provides a table caption, and a numbered
table in \LaTeX;
\item column alignment (\code{"r"} or \code{"l"}) for the table columns
...
~~~\code{V2}--- values 1:11 correspond to the quarter-hour times 8:00 pm through 10:30 pm;\\
...
\ixon{data frame!creating} 
\ixon{arrays!creating}
...
\verb|TV.dat$Day <- factor(TV.dat$Day)| etc., and only supply labels for the
first variable.
<<TV-df>>=
TV_df <- tv_data
...
a 4-way array.
In the lines below, we rely on the facts that 
...
\ixoff{data frame!creating} 
\ixoff{arrays!creating}
...
<<cleanup02, include=FALSE, purl=FALSE>>=
