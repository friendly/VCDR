...
in various formats: \ix{case form} \ix{frequency form} \ix{table form}
...
\term{Matrices} are rectangular arrays like a traditional table, composed of vectors in their columns
or rows.
\term{Arrays} add additional dimensions, so that, for example, a 3-way table can be represented
...
data structure is a \emph{list}, a generic vector that can contain \ix{list}
...
considered in more detail in \secref{sec:ordered}. \ix{variable!ordered}
...
\subsection{Case form} \ixon{case form}
...
\ixon{variable!ordered} \ixon{factor}
...
their labels. For example, in the \data{HairEyeColor} table, hair color and eye color are ordered arbitrarily. \ixon{table form!ordering}
...
using the \func{mosaicplot} function from the \basepkg{graphics} package. \ix{mosaic display}
...
\ix{data frame!subsetting} \ix{subsetting}
...
\ixon{table form!collapsing} \ixon{factor}
...
factors \emph{not} mentioned in the formula. \ix{frequency form}
...
using the \func{sum} function. \ixon{table form}
...
\ix{data frame!converting} \ix{case form!converting} 
\ix{frequency form!converting} \ix{table form!converting}
\begin{table}[htb]
...
\ix{table form!publishing} \ix{LaTeX}
...
\ixon{data frame!creating} \ixon{arrays!creating}
...
\ixoff{data frame!creating} \ixoff{arrays!creating}

\subsection{Subsetting and collapsing}
\ixon{table form} \ixon{subsetting} \ixon{data frame!subsetting}
For many purposes,
...
\ixoff{table form} \ixoff{subsetting} \ixoff{data frame!subsetting}
