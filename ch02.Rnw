...
a double-blind clinical trial investigating a new treatment for rheumatoid arthritis with 84 patients.\footnote{This data can be created using: \code{library(vcd); write.table(Arthritis, file = "Arthritis.csv", quote = FALSE, sep = ",")} }
...
<<subset7>>=
subset(Arthritis, Sex == "Female" & Age > 68, c(Treatment, Improved))
@ 
...
<<>>=
subset(Arthritis, Sex == "Female" & Age > 68, -c(Age, ID))
@ 
...
described in \exref{ex:horsekick1} and containing the frequencies
