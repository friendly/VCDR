...
pun_lor <- loddsratio(Freq ~ memory + attitude | age + education, data = Punishment)
@
...
titanic_lor1 <- loddsratio(~ Survived + Age | Class + Sex, data = Titanic2)
titanic_lor1
@
...
<<titanic-lor2>>=
titanic_lor2 <- loddsratio(~ Survived + Sex | Class + Age, data = Titanic2)
titanic_lor2
@
