...
taken as \code{NA} because these zeros are \term{structural zeros}
\ix{zeros!structural}
Problems of zero frequencies in frequency tables are discussed
in more detail in \secref{sec:loglin-zeros}.
}
<<titanic-lor2>>=
