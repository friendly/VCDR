...
They have the advantage of making it easier to ``read'' the differences among the response proportions in relation to combinations of
...
shade to highlight the last level (Rejected). We want to highlight Admitted, so we reverse
...
@
In \figref{fig:berkely-doubledecker}, it is easy to see the effects of both \var{Dept} and \var{Gender} on 
...
\figref{fig:titanic-doubledecker} shows the doubledecker plot for the \data{Titanic} data, where (by default), the 
levels of the response (\var{Survived}) are shaded in increasing grey levels.
...
\begin{figure}
  \hfill
    \includegraphics[height=.4\textwidth,keepaspectratio=true]{ch05/fig/lor1} \hfill
    \includegraphics[height=.4\textwidth,keepaspectratio=true]{ch05/fig/lor2}
  \hfill
    \label{fig:lor}
    \caption{Generalized odds ratios for an $R \times C$ table. Left: local odds ratios for adjacent categories. Right: odds ratios
    with respect to a reference category (the last). Each log odds ratio is a contrast of the log frequencies.}
\end{figure}
