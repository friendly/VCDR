...
\end{table}
  \begin{itemize*}
    ...
    \begin{ans}
    These notes use a later version of the Lahman package with the code abov, so the numbers
    don't correspond to those in the table.
    \end{ans}
    ...
    \begin{ans}
<<ex5_3b>>=
mosaic(t(basehands), direction=c("h", "v"), shade=TRUE)
mosaic(basehands, shade=TRUE, labeling=labeling_residuals())
@
