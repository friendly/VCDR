...
reshapes thse into an array whose dimensions are given in the vector \code{dim}.
...
first argument allowing repetitions (\code{rep=TRUE}).
...
\begin{description*}
  \item [\code{header}] (default: \code{FALSE}) indicates whether the first line contains
variable names;
...
response variable, and the others as possible predictors.
  ...
  \begin{enumerate*}
    ...
    flattened display of the frequencyes in the 4-way table.  Choose the
...
that male and female births were equally likely, the probablity of 82 years of
...
For now, we simply display a plot of the probabilty of a male birth over time.
...
\hlkwd{with}\hlstd{(Arbuthnot, \{}
  ...
  \hlstd{Arb.smooth} \hlkwb{<-} \hlkwd{with}\hlstd{(Arbuthnot,} \hlkwd{loess.smooth}\hlstd{(Year,prob))}
...
parameter $\sum \lambda_i$.
  ...
  and constant variance $\frac14$.  Hence, the sqrt transformation is often recommended
...
\figref{fig:dbinom2-plot}.  With \func{xyplot}, the plot style is controled
...
difficult to contruct with \pkg{lattice}, and (b) is difficult to read, because
...
to the mean, and therefore greater applicability of the Possion distribution.
...
\TODO{This subsection should be omittted, unless we extend the calculation and plotting 
methods.}
...
\begin{enumerate}
  ...
  \item Continue \exref{ex:saxfit} on the distribution of male chilren in families
...
\end{knitrout}
  ...
  \begin{enumerate*}
    \item Read these data into \R and construct a 1-way table of frequencies of counts
...
% the table is better
  ...
  \begin{enumerate*}
    ...
    \item Try fitting the negative binomial distribution, and use the \func{distplot} to diagnose
...
\TODO{Add \data{Depends} to \pkg{vcdExtra}; perhaps promote this table to an introductory
