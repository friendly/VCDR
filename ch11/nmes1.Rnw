...
@
As before, it is useful to precede the formal analysis with a variety of exploratory plots.
...
\begin{itemize*}
  \item Have all important predictors and control variables been included in the model?
  \item Are quantitative predictors represented on the correct scale (via transformations or nonlinear terms)
...
to the standard Poisson regression model as we expect
...
\figref{fig:nmes2-eff3}, it can be seen that for people with poor health
...
office visits increase with chronic conditions up to a point and then decrease---with 
a quadratic, what goes up must come down, the same way it went up!  This makes no sense here,
...
An alternative approach to handle nonlinearity is to use
regression splines
...
\code{nmes.gamnb}.
These plots use \func{rainbow} colors (in HCL space) to depict the predicted values of office visits.
...
<<nmes3-rsm, h=6, w=6, out.width='.5\\textwidth', cap='Fitted response surfaces for the relationships among chronic conditions,  number of hospital stays, and years of education to office visits in the generalized additive model, nmes.gamnb.', echo=FALSE, fig.pos="!b">>=
library(rsm)
library(colorspace)
...
level off. For those with no chronic conditions, the effect of hospital stays rises to a higher level
