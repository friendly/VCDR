...
\subsection{Analyzing correlations: HE plots}
\ixon{hypothesis-error plot}
...
for each term in an MLM in relation to error covariances (E) using data ellipsoids in the
...
reasonably useful overviews on the relationships among the $y$ and $x$ variables.
\ixoff{hypothesis-error plot}

\subsection{Analyzing associations: Odds ratios and fourfold plots}
...
corresponding to the distinctions among visits.  While we are at it, we create factors for two of the
predictors.
...
\footnote{A similar plot can be obtained using \texttt{cotabplot(\textasciitilde
  practitioner + place + chronicf + insurance | gender, tab, panel =
  cotab\_loddsratio)}.}
...
\begin{equation*}
\eta (\vec{x}) = \sum_{k=1}^p  \mat{H}_k  \vec{\beta}_k \vec{x}_k \comma
\end{equation*}
...
complex models for physician office visits, including interactions and nonlinear terms.
...
%%% car::linearHypothesis generated an error with the latest version, car_2.1-0 
%%% See: issue #53
