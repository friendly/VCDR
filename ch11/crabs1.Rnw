...
the number of satellites for each female? Or, perhaps better, how do unattached males choose among
available females?
...
\citet[\S 4.3]{Agresti:2013} analyzes the number of satellites using count data
...
<<crabs1-scats, h=5, w=6, out.width='.49\\textwidth', cap='Scatterplots of number of satellites vs.\ width and weight, with lowess smooths.',size="footnotesize">>=
