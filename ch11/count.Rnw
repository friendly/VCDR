...
to which some annotations can be added using standard plotting tools,
such as the mean or an interval showing the variance (e.g., 
with a range of one standard deviation).
...
necessary to avoid overplotting and a smooth nonparametric curve can show possible nonlinearity.
...
To start analysis, we fit the Poisson model using all 
predictors---\code{female}, \code{married}, \code{kid5}, 
\code{phdprestige}, and
...
a 17\% increase, while each additional child multiplies articles by
0.831, a 16.88\% decrease. 
\ix{effect plot}
...
dramatic, particularly for those whose mentor was truly prolific.
