...
\begin{figure}[htb]
  \centering
  \includegraphics[width=.6\textwidth]{ch11/fig/ExcessZeros}
...
\begin{equation}\label{eq:zip-logit}
 \logit (\pi_i) = \vec{z}_i\trans \vec{\gamma}
                = \gamma_0 + \gamma_1 z_{i1} + \gamma_2 z_{i2} +
                \cdots + \gamma_q z_{iq} \period
\end{equation}

  \item A Poisson model for the other class (e.g., ``publishers''),
    for whom the observed count may be 0 or
...
\Pr ({y_i} \given \vec{x}, \vec{z}) &=& (1 - {\pi _i}) \times
                                        \left[\frac{{{\mu
                                        _i}^{{y_i}}{e^{ - {\mu
                                        _i}}}}}{{{y_i}!}} \right],
                                        \quad\quad y_i \ge 0 \comma \nonumber
\end{eqnarray}
...
\ix{zero-inflated Poisson!plotting}
\ix{bar plot}
...
\ixon{hurdle model}
\ixon{zero-altered model}
A different class of models capable of accounting for excess zero counts is the
\textbf{\emph{hurdle model}} (also called the \textbf{\emph{zero-altered model}})
...
\times \left[\frac{{{\mu _i}^{{y_i}}{e^{ - {\mu _i}}}}}{{{y_i}!}}
                                        \right], \quad\quad y_i \ge 0 \period \nonumber
\end{eqnarray}
...
A different, and simpler idea is illustrated here using what is called a \term{spineplot}
...
These plots are provided by \func{cdplot} in the \basepkg{graphics} package and a similar
...
predictors. 
%\DONE{Fixup use of color in \exref{ex:crabs1} so it doesn't cause a problem here}
...
par(op)
@
\ixoff{zero-frequencies!plotting}
\end{Example}
