...
assign and print by surrounding the assignment with \code{()}. \ix{assignment operator}
...
Other useful functions for creating vectors are: \ix{:}
\begin{itemize*}
...
Matrices can also be created or enlarged by ``binding'' vectors or matrices together \ix{vector!binding} ix{matrix!binding}
by rows or columns:
\begin{itemize*}
...
using \func{t}: \ix{matrix!transposing}
