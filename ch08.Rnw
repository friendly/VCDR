...
and called \term{Poisson regression}.
...
polynomials, allowing linear, quadtratic, ... effects, assuming
...
observed freqencies perfectly, but offers no simpler description or
...
of homogeous association for a three-way table is specified
as 
<<loglm1, eval=FALSE>>=
loglin(mytable, margin=list(c(1, 2), c(1, 3), c(2, 3)))
@

The function \func{loglm} in \pkg{MASS} provides a more convienient
...
\chisq = \sum_i^N \frac{( n_i - \widehat{m}_i )^2}{\widehat{m}_i}
\comma
\end{equation}
...
is not substantitively reasonable here, 
...
Next, consider the
model of conditional independence of gender and admision given department,
...
<<berk-glm3-mosaic, fig.pos='!htb', h=6, w=6, out.width='.7\\textwidth', cap='Mosaic display for the model \\code{berk.glm3}, allowing an association of gender and admission in Department A.', scap='Mosaic display for the model berk.glm3'>>=
...
Even when there is no respopnse variable, the logit representation
...
\section{Zero frequencies}\label{sec:zeros}
...
the locations of the zero cels, and non-zero elsewhere.%
\footnote{
...
\end{itemize*}
In complex, sparce tables, a sensitivity analysis, comparing different approaches can help determine if the substantitive conclusions vary with the approach to zero cells.
...
As usual, we abbreviate the table varibles concern, age, gender
...
health.glm1 <-glm(Freq ~  concerns + age * gender, data=Health, 
                  subset=(Freq>0), family=poisson)
@
Neither of these fits the data well.  To concerve space, we show only the
...
These are incoprporated into the next model:
<<health4>>=
...
<<health5, size='footnotesize', echo=2>>=
op <- options(width=80)
summary(health.glm2)
options(op)
@
...
\section{Three-way and higher-dimensional tables}
...
vcdExtra::summarise(vis.hetmodels)
@
...
The substantitive interpretation of this example is that 
