...
<<include=FALSE>>=
#opts_chunk$set(engine='R',fig.height=6,fig.width=7,results='hide',fig.keep='none',echo=TRUE)
opts_chunk$set(engine='R',fig.height=6,fig.width=7,results='hide',echo=TRUE)
@


<<include=FALSE>>=
opts_chunk$set(fig.path='fig/vcd-tut')
@
...
the \func{str} function, or view the names of its components with the
\func{names} function.
...
\func{matrix} uses the elements supplied by \emph{columns} in the
result, unless you specify \code{byrow=TRUE}.
<<table-form2,results='markup'>>=
...
eye colors by indexing. Again \func{str} is your friend.

<<reorder1,results='markup'>>=
...
\func{ordered} (and others) can be useful. 
...
<<Arthritis,echo=FALSE,fig.height=6,fig.width=7,results='hide',include=FALSE>>=
...
%There is one subtle ``gotcha'' here:  \func{aperm} returns an object of class \class{"array"},
...
%The solution is to reassign the \code{class} of the result of  \func{aperm}.
%
%<<reorder4,results='markup', keep.source=TRUE>>=
%class(UCBAdmissions)
%class(UCB)
...
%@
%
\subsection[structable()]{\func{structable}}\label{sec:structable}
For 3-way and larger tables
the \func{structable} function in \pkg{vcd} provides a convenient and flexible tabular display.
The variables assigned to the rows and columns of a two-way display can be specified
by a model formula.
<<structable,results='markup'>>=
...
\func{mosaic} (not shown here).
<<structable1,eval=FALSE>>=
HSE < - structable(Hair+Sex ~ Eye, HairEyeColor)   # save structable object
mosaic(HSE)                                        # plot it
@

\subsection[table() and friends]{\func{table} and friends}\label{sec:table}

You  can  generate frequency  tables from factor variables  using the  \func{table} function,  tables  of
proportions using  the \func{prop.table} function,  and marginal  frequencies using
\func{margin.table}.
...
\func{table}  ignores missing values by default. 
...
The \func{xtabs} function allows you to create crosstabulations of data using formula style input.
...
\subsection[Collapsing over factors]{Collapsing over table factors: \func{aggregate}, \func{margin.table} and \func{apply}}
...
\subsection[Collapsing levels]{Collapsing table levels: \func{collapse.table}}
...
to a data.frame with \func{as.data.frame}.%
\footnote{
...
the \code{responseName} argument, defaulting to \code{Freq}. This is the inverse of \func{xtabs}.

\Example 
Convert the \code{GSStab} in table form to a data.frame in frequency form.
<<convert-ex1,results='markup'>>=
...
\footnote{
\func{table} does not allow a \code{data} argument to provide
...
but \func{attach} leaves the variables in the global environment,
while \func{with} just evaluates the \func{table} expression in a
temporary environment of the data.
}
<<convert-ex2,results='markup'>>=
...
Marc Schwartz provided code for \func{expand.dft} on the  Rhelp
...
head(tv.data,5)
@
For a local file, just use \func{read.table} in this form:
<<tv2,eval=FALSE>>=
...
the  \func{CrossTable}  function in  the \pkg{gmodels}  package produces  cross-tabulations
...
For 2-way tables you can use \func{chisq.test} to test independence of the row
...
from the odds ratios for each $2 \times 2$ table (\func{oddsratio}), and
tested 
by  using
...
each  department  with  fourfold  displays  using  \func{fourfold}.  The cell
...
<<fourfold1,fig.height=6,fig.width=9,results='hide',include=FALSE>>=
...
Another \pkg{vcd} function, \func{cotabplot}, provides a more general approach
to visualizing conditional associations in contingency tables,
similar to trellis-like plots produced by \func{coplot} and lattice graphics.
...
an alternative visualization is a \func{doubledecker} plot.
...
<<doubledecker, fig.height=5,fig.width=9,include=FALSE>>=
...
The \func{CMHtest} function in \pkg{vcdExtra} now calculates these various
...
The  \func{assocstats}  function in \pkg{vcd}  calculates  the   $\phi$
...
The
\func{Kappa} function in the \pkg{vcd} package calculates Cohen's $\kappa$ and weighted
$\kappa$ for a square two-way table with the same row and column categories \citep{Cohen:60}.%
\footnote{ 
Don't confuse this with \func{kappa} in base \proglang{R} that computes something
...
error (ASE).  A \func{confint} method for \code{Kappa} objects provides confidence intervals.
<<kappa,results='markup'>>=
(K <- Kappa(SexualFun))
confint(K)
@

A visualization of agreement, both unweighted and weighted for degree of departure
from exact agreement is provided by the \func{agreementplot} function.
...
<<agreesex,fig.height=6,fig.width=7,results='markup',include=FALSE>>=
...
\footnote{
	...
	the analogous \func{glm} model-based methods (\secref{sec:CMH}),
	and the more general RC models which can be fit using \func{gnm}.
...
The resulting \code{ca} object can be plotted just by running the \func{plot}
method on the \code{ca} object, giving the result in
\figref{fig:ca-haireye}.  \func{plot.ca} does not allow labels for dimensions;
these can be added with \func{title}.
...
<<ca-haireye,echo=TRUE,eval=FALSE,fig.keep='high'>>=
...
You can  use the  \func{loglm}  function in  the \pkg{MASS}  package to fit log-linear
models.  Equivalent models can also be fit (from a different perspective) as generalized
linear models with the \func{glm}  function using the \code{family='poisson'} argument,
...
The visualization methods for these models were originally developed for models fit using \func{loglm},
so this approach is emphasized here.  Some extensions of these methods for models
fit using \func{glm} and \func{gnm} are contained in the \pkg{vcdExtra} package
...
The \textbf{Model formula} column shows how to express each model for \func{loglm} in \proglang{R}.%
\footnote{
For \func{glm}, or \func{gnm}, with the data in the form of a frequency data.frame, the same model is specified
...
<<loglm-hec2,results='markup'>>=
## Conditional independence
hec.2 <- loglm(~(Hair + Eye) * Sex, data=HairEyeColor)
hec.2
@
<<loglm-hec3,results='markup'>>=
...
A set of models can be compared using the \func{anova} function.

<<loglm-anova,results='markup'>>=
...
\subsection[Fitting with glm() and gnm()]{Fitting with \func{glm} and \func{gnm}}\label{sec:glm}
The \func{glm} approach, and extensions of this in the \pkg{gnm} package allows a
much wider class of models for frequency data to be fit than can be handled by
\func{loglm}.  Of particular importance are models for ordinal factors and for
...
\func{loglm}. These are similar in spirit to the 
...
\func{anova} when those models are listed from smallest to largest.
...
In the \pkg{gnm} package, these models may be fit using the \func{Mult}
to specify the multiplicative term, and \func{instances} to specify several
such terms.
...
For a two-way table, \func{mosaic} fits a model of independence, $[A][B]$
or \verb|~A+B| as an \proglang{R} formula.
For $n$-way tables, \func{mosaic} can fit any \loglin\ model, and can also be
used to plot a model fit with \func{loglm}.
...
\func{mosaic} provides a wide range of options for the directions of splitting,
...
\setkeys{Gin}{width=0.5\textwidth}
<<art21,fig.height=6,fig.width=7,echo=FALSE>>=
mosaic(art, gp = shading_max, split_vertical = TRUE, 
       main="Arthritis: gp = shading_max")
@
<<art22,fig.height=6,fig.width=7,echo=FALSE>>=
...
When you have fit a \loglin\ model using \func{loglm}, 
and saved the result (as a \code{loglm} object) the simplest way to display the
results is to use the \func{plot} method for the \code{loglm} object.
...
<<hec1,fig.height=6,fig.width=7,echo=FALSE>>=
plot(hec.1, main="model: [Hair][Eye][Sex]")
@
<<hec2,fig.height=6,fig.width=7,echo=FALSE>>=
plot(hec.2, main="model: [HairSex][EyeSex]")
@
<<hec3,fig.height=6,fig.width=7,echo=FALSE>>=
plot(hec.3, main="model: [HairSex][EyeSex]")
@

Alternatively, you can supply the model formula to \func{mosaic}
with the \code{expected} argument.  This is passed to \func{loglm},
...
on the vertical axis and \code{Network} on the horizontal,
  ...
  \func{level} to abbreviate them.  \func{mosaic} also supports a 
...
The following call to \func{mosaic} produces \figref{fig:TV-mosaic}:
...
\begin{figure}[htb]
\begin{center}
<<TV-mosaic,fig.height=6,fig.width=6,echo=FALSE>>=
dimnames(TV2)$Time <- c("8", "9", "10")     # re-level for mosaic display
mosaic(~ Day + Network + Time, data=TV2, expected=~Day:Time + Network, 
         legend=FALSE, gp=shading_Friendly)
@
...
mosaic(~ Day + Network + Time, data=TV2, 
         ...
         legend=FALSE, gp=shading_Friendly)
@
\subsection[Mosaics for glm() and gnm() models]{Mosaics for \func{glm} and \func{gnm} models}\label{sec:mosglm}
The \pkg{vcdExtra} package provides an additional method, \func{mosaic.glm} 
for models fit with \func{glm} and \func{gnm}.%
\footnote{
Models fit with \func{gnm} are of \code{class = c("gnm", "glm", "lm")},
...
residuals printed in cells).  Note that the \code{formula} supplied to \func{mosaic}
...
<<mental-plots1,echo=FALSE,fig.height=6,fig.width=6>>=
long.labels <- list(set_varnames = c(mental="Mental Health Status", ses="Parent SES"))
mosaic(indep, ~ses+mental, residuals_type="rstandard",  
     labeling_args = long.labels, labeling=labeling_residuals,
     main="Mental health data: Independence")
@
<<mental-plots2,echo=FALSE,fig.height=6,fig.width=6>>=
long.labels <- list(set_varnames = c(mental="Mental Health Status", ses="Parent SES"))
mosaic(linlin, ~ses+mental, residuals_type="rstandard", 
     labeling_args = long.labels, labeling=labeling_residuals, suppress=1, 
     gp=shading_Friendly, main="Mental health data: Linear x Linear")
...
are specified by the \func{Mult}.
...
(\func{Inv}) and the exponential of a predictor (\func{Exp}).
...
(\func{mosaic}), as well as association plots (\func{assoc}), sieve diagrams
(\func{sieve}), as well as tabular displays (\func{structable}).
...
\verb|set_labels| options in \func{mosaic} as follows:

<<reorder3b,results='hide', tidy=FALSE>>=
...
as follows.  \func{spine} takes a formula of the form 
...
<<spine1,fig.keep='none',results='markup'>>=
(spine(Improved ~ Age, data = Arthritis, breaks = 3))
(spine(Improved ~ Age, data = Arthritis, breaks = "Scott"))
@

\setkeys{Gin}{width=0.49\textwidth}
<<spine2,echo=FALSE,fig.height=6,fig.width=6>>=
(spine(Improved ~ Age, data = Arthritis, breaks = 3))
@
<<spine3,echo=FALSE,fig.height=6,fig.width=6>>=
(spine(Improved ~ Age, data = Arthritis, breaks = "Scott"))
@
...
\begin{figure}[htb]
\begin{center}
<<cdplot,fig.height=6,fig.width=6>>=
...
across the horizontal axis, e.g., with a \func{rug} plot.
\figref{fig:cd-plot} uses \func{cdplot} from the \pkg{graphics} package
...
For complex \func{glm} models with interaction effects, the \pkg{effects}
...
for model-smoothed plots of categorical responses in \func{glm} models.
...
<<donner2a, fig.keep='none', eval=FALSE>>=
...
<<donner2b, fig.keep='none', eval=FALSE>>=
# separate quadratics
ggplot(Donner, aes(age, survived, color = sex)) +
  geom_point(position = position_jitter(height = 0.02, width = 0)) +
...
\begin{figure}[htb]
\begin{center}
<<donner3a,echo=FALSE,fig.height=6,fig.width=6, results='hide'>>=
ggplot(Donner, aes(age, survived, color = sex)) +
  geom_point(position = position_jitter(height = 0.02, width = 0)) +
  stat_smooth(method = "glm", family = binomial, formula = y ~ x,
           alpha = 0.2, size=2, aes(fill = sex))
...
@
<<donner3b,echo=FALSE,fig.height=6,fig.width=6, results='hide'>>=
# separate quadratics
ggplot(Donner, aes(age, survived, color = sex)) +
  geom_point(position = position_jitter(height = 0.02, width = 0)) +
  stat_smooth(method = "glm", family = binomial, formula = y ~ poly(x,2),
