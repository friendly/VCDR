...
opts_chunk$set(engine='R',fig.height=6,fig.width=7,results='hide',fig.show='hide',echo=TRUE)
@
...
<<Arthritis,fig.keep='high',fig.height=6,fig.width=7,results='hide',include=FALSE>>=
...
<<fourfold1,fig.keep='high',fig.height=6,fig.width=9,results='hide',include=FALSE,echo=TRUE>>=
...
<<doubledecker, fig.keep='high', fig.height=5,fig.width=9>>=
...
<<oddsratio,fig.keep='high',fig.height=6,fig.width=6>>=
...
<<agreesex,fig.keep='high',fig.height=6,fig.width=7,results='markup'>>=
...
<<ca-haireye,echo=TRUE,fig.keep='high'>>=
...
<<loglm-summarise,results='markup',message=FALSE>>=
vcdExtra::summarise(loglmlist(hec.1, hec.2, hec.3))
@
...
\begin{figure}[htb]
\begin{center}
<<TV-mosaic,fig.keep='high',fig.height=6,fig.width=6,echo=FALSE>>=
dimnames(TV2)$Time <- c("8", "9", "10")     # re-level for mosaic display
mosaic(~ Day + Network + Time, data=TV2, expected=~Day:Time + Network, 
         legend=FALSE, gp=shading_Friendly)
@
...
<<TV-mosaic2,fig.keep='none',results='hide'>>=
dimnames(TV2)$Time <- c("8", "9", "10")     # re-level for mosaic display
mosaic(~ Day + Network + Time, data=TV2, expected=~Day:Time + Network, 
         legend=FALSE, gp=shading_Friendly, 
