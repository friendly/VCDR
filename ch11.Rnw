...
\begin{itemize}
 \item provides for simple, linear relations between the response and the predictors
...
one model is closer. It assumes that both models are \emph{strictly} non-nested, which
is not obvious to see in all cases---for example, some models may neither be nested
...
@
  ...
  \exercise The data set \data{AirCrash} in \pkg{vcdExtra} was analysed in \labref{lab:mosaic-crash} and \labref{lab:ca-crash}
