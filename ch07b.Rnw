...
\chapter{Models for Polytomous Responses}\label{ch:polytomous}
%\input{ch07b/vtoc}		% visual table of contents
...
\label{sec:ordinal}
<<propodds, child="ch07b/propodds.Rnw">>=
@

\section{Nested dichotomies}\label{sec:nested}
<<nested, child="ch07b/nested.Rnw">>=
@

\section{Generalized logit model}\label{sec:genlogit}
<<genlogit, child="ch07b/genlogit.Rnw">>=
@

\section{Chapter summary}\label{sec:ch07b-summary}
\input{ch07b/summary}

\section{Lab exercises}\label{sec:ch07b-exercises}
<<exercises07b, child="ch07b/exercises.Rnw">>=
@

% Cleanup local variables
<<cleanup07b, size='footnotesize', include=FALSE>>=
# detach(package:ggtern)  ## detach any masking packages
detach(package:rms)
.locals$ch07b <- setdiff(ls(), .globals)
.locals$ch07b
remove(list=.locals$ch07b[sapply(.locals$ch07b,function(n){!is.function(get(n))})]) 
