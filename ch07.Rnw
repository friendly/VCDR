...
<<arth-logi-hist, h=5, w=6, out.width='.6\\textwidth', cap='Plot of the Arthritis treatment data, showing the conditional distributions of the 0/1 observations of the Better response by histograms ang boxplots.', echo=-1, size='footnotesize' >>=
...
<<arth-test2>>=
vcdExtra::summarise(arth.logistic)
@
...
\label{sec:ordinal}
<<propodds, child="ch07/propodds.Rnw">>=
@
...
for ( kids in c("absent", "present") ) {
  ...
  lines(Hinc, dat$full, lwd=3, col="red",   lty=3)
  if (kids=="absent") {
...
#.locals$ch07
remove(list=.locals$ch07[sapply(.locals$ch03,function(n){!is.function(get(n))})]) 
@
