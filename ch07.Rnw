...
\begin{figure}
 \centering
 \includegraphics[width=\textwidth]{ch07/fig/goverview-R3.jpg}
...
random component.  As we will see in \chref{ch:polytomous}, 
these methods extend readily to
...
in the context of constructing and interpreting multiple logistic regression models.  
...
\ix{response variables!discrete}
...
\ixon{linear regression}
\ixon{regression!linear}
...
\exref{ex:arthrit6}--\exref{ex:arthrit8}: The ``Better'' response
represents a positive effect of some arthritis medicament, given age.
...
\ixoff{linear regression}
\ixoff{regression!linear}
...
\pi (x) =
\mbox{logit}^{-1}[ \pi(x) ] =
\frac{1}{1 + \exp [- (\alpha + \beta x) ]} \period
\end{equation}
\ix{probit models}
\ix{regression!probit}
...
\begin{seriate}
 ...
 the log odds or a multiplicative model for the odds.
\end{seriate}
}
...
\ixon{logistic regression!interpretation}
...
\ixoff{logistic regression!interpretation}
\ixon{logistic regression!plotting}
...
logistic curve arises from their distribution across the range of a predictor.
For didactic purposes this can be seen more readily by plotting the conditional distributions
of $f(x \given y), y \in \{0,1\}$ as a histogram, boxplot, or density plot.
The function \func{logi.hist.plot} in the \Rpackage{popbio} is a nice implementation of
...
with the $\beta / 4$ rule of thumb, which gives 0.0492/4 = \Sexpr{0.0492/4}.
...
than the null model $\logit(\pi) = \alpha$, which includes only the
...
\ix{deviance}
The more direct test compares the deviance%
\footnote{
  The deviance is basically defined as $-2$ times the log-likelihood ratio of
  some reduced model to the full model. Two nested models can thus be
  compared by computing the difference of the corresponding
  deviances. If the larger model has $k$ more parameters than the
  reduced one, this difference follows a chi-squared distribution with $k$
  degrees of freedom.
}
of the fitted model to the deviance
...
@
  ...
  of the size of the residual deviance, which is given by the function
...
\ixoff{logistic regression!model tests}
...
\ix{jitter}
\ix{non-parametric regression}
\ix{graphical elements!lines}
\ix{confidence bands}
\begin{itemize*}
  ...
  is also added to the plot to give some indication of possible nonlinearity in
...
using \code{lines}.  A graphics trick is to use a transparent color for
...
more straightforward to produce using \pkg{ggplot2}.
...
\end{itemize}
\ixoff{case form}
\ixoff{data!case form}
\ixoff{frequency form}
\ixoff{data!frequency form}
...
also analyzed by \citet{Lavine:91}.
...
\ixon{logistic regression!multiple}
\ixon{regression!multiple logistic}
...
\begin{equation*}
  \logit ( \pi_{i}) = \alpha + \beta_1 x_{i1} + \beta_2 x_{i2} + \beta_3 x_{i3}
\end{equation*}
...
with $x_2=0$ and $x_3=0$---that is, females receiving the placebo.  To make the intercept
...
treated group.  \(e^{ \beta_3 }\) gives the odds of
...
Male & Treated & \(\alpha + \beta_2 + \beta_3\) & \(e^{\alpha + \beta_2 + \beta_3}\)  \\  \hline
\end{tabular}
}
\end{center}
...
\end{Example}
\ixoff{logistic regression!multiple}
\ixoff{regression!multiple logistic}

\subsection{Conditional plots}\label{sec:logist-condplots}
\ix{logistic regression!plotting}
...
and \pkg{ggplot2} in the style of earlier examples (Examples \ref{ex:arthrit7}, \ref{ex:arthrit7}, and \ref{ex:nasa-temp}) become more tedious with multiple predictors.  
...
The idea of effect plots is quite simple but very general and handles models
...
low-order relatives, but with all other variables controlled (or
``adjusted for'').

% \TODO{DM: what is the difference between ``controlling'' and
%   ``adjusting'' variables?}

\subsubsection{The score model matrix\hard}

\ixon{score model matrix}
\ixon{matrices}
...
the trade-off between plots on the logit scale, which have a simple representation
in terms of additive effects, and plots on the probability scale which are
...
\subsubsection{Partial residuals}

\ixon{residuals!partial}
We noted earlier that for discrete response data, it is usually important to display
the \emph{data} in some fashion, along with the fitted relationship.
Conditional and full-model plots do this by jittering the binary values at 0 and 1
so you can see where the data exists.

The \Rpackage{effects} takes this idea further, by allowing the display of
\term{partial residuals}.  Letting $\vec{r}$ denote the vector of residuals for
a given model (see \secref{sec:logist-resids} for details), the partial
residuals $\vec{r}_j$ pertaining to predictor $\vec{x}_j$ are defined as
\begin{equation*}
\vec{r}_j = \vec{r} + \widehat{\beta}_j \vec{x}_j \period
\end{equation*}
These are a natural extension of residuals in simple regression to the multiple regression
setting, in that the slope of a simple regression of $\vec{r}$ on $\vec{x}$ is equal to
the value of $\widehat{\beta}_j$ in the full multiple regression model \citep{Cook:93}.
Adding partial residuals to an effect plot (together with a non-parametric smoothing)
can help to visualize lack of fit or misspecification of the response mean
attributable to continuous predictors, such as a nonlinear relation or an omitted interaction
\citep{FoxWeisberg:2015:eff}.
...
model that was fit in \exref{ex:arthrit-mult}.  \func{allEffects} is used to
...
Partial residuals (for quantitative predictors) must be requested in the call
to \func{allEffects} or \func{Effect}.
<<arth-eff1>>=
library(effects)
...
for each of the model terms. For example the main effect for \var{Sex} is shown below;
the associated score \code{model.matrix} illustrates how \var{Sex} is varied over its range,
while \code{Age-50} and \code{Treatment} are fixed at their average values in the data.
<<>>=
arth.eff2[["Sex"]]
arth.eff2[["Sex"]]$model.matrix
@

% No \code{xlevels} argument was specified, so by default
...
% \DONE{DM: Showing and explaining one of the score model matrices,
%   e.g. \code{arth.eff2[[1]]\$model.matrix}, would make clear what ``varying  over a range'' and ``fixing at typical values'' means.}
...
term, which are just the main effects in this model.  The call below produces
...
plot(arth.eff2, rows = 1, cols = 3,
     type="response", residuals.pch = 15)
@
\noindent The smoothed loess curve for the partial residuals with respect to age show a hint of
nonlinearity, but perhaps not enough to worry about.

\ix{loess}

You can quite easily also produce effect plots for several predictors jointly, or
full-model plots by using all predictors in the model
in a call to \func{Effect}, as shown in the call below. 
<<arth-full>>=
...
\subsection{Simple models: Group comparisons and effect plots}\label{sec:complex1}
...
\subsection{More complex models: Model selection and visualization}\label{sec:complex2}

\ix{interactions}
\ix{nonlinearity}
Models with more predictors or more complex terms (interactions, nonlinear terms)
...
nonlinear relationships with the response and give an illusion of a comfortable
interpretation.
...
\citet[\C 6]{Agresti:2013}, and extensively in \citet[Chapters 10--13]{Harrell:2001}.
...
\mat{H} = {\mat{X}}^\star
{( {\mat{X}^\star}\trans {\mat{X}}^\star )}^{-1} {\mat{X}^\star}\trans \comma
\end{equation*}
where \({\mat{X}} ^\star = {\mat{V}}^{1/2} \mat{X}\), and \(\mat{V}  =
\diag [ \hat{\vec{p}} ( 1 - \hat{\vec{p}})] \).  

\ix{regression!leverage}
\ix{leverage}
As in OLS,
...
\ix{influence!plotting}
Basic diagnostic plots are provided by the \func{plot} method for a
\class{glm} model object. These are easy to do, but the results for discrete response data
are often unsatisfactory.
The \Rpackage{car} contains a variety of enhanced and extended functions
...
straightforward indicators of the adequacy of a particular model,
...
\begin{equation*}
\vec{r}_j^{\star} = \vec{r} + \beta_j \vec{x}_j \period
\end{equation*}
...
However, any nonlinear trend will be shown in the pattern of the points,
...
\vec{r}_j^{\star} = \mat{V}^{-1} \vec{r} + \beta_j \vec{x}_j \period
...
(\secref{sec:logist-condplots}), and are most useful when there are several quantitative predictors,
...
check for a nonlinear relationship of survival with age as follows, giving \figref{fig:donner-cr1}.
...
@
\ix{loess}
\ix{conditional plots}
The smoothed loess curve in this plot closely resembles the trend we saw in the conditional
plot for age by sex (\figref{fig:donner1-cond3}), suggesting the need to include a nonlinear
...
\ix{partial residual plot}
For comparison, you can see the result of allowing for a nonlinear relationship in
...
@
Except possibly at the extreme right, this plot
(\figref{fig:donner-cr2}) shows no indication of a (further)
...
the residuals from
the regressions of $\vec{y}$, and of $\vec{x}_i$, on all the other $x$s
excluding $\vec{x}_i$.
...
cannot easily show influential observations or nonlinear relationships.  Added-variable
...
have \term{joint influence}, which greatly exceeds their individual influential.
...
color for  the plotting symbol to distinguish those who survived vs.\ died,
...
\ix{added-variable plot!properties}
...
par(op)
@
%\DONE{DM: red is rather flashy - maybe use magenta instead? -- pink is better}
...
\ix{logistic regression!influential cases}
...
Case 84, however, has high leverage and a large residual, so it is possibly influential on the
