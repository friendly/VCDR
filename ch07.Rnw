...
% arth.eff2[[1]]
% @
\TODO{DM: Showing and explaining one of the score model matrices,
...
Whereas basic diagnostic plots are provided by the \func{plot} method for a
\class{glm} model object, the \Rpackage{car} contains a variety of extended functions
...
par(op)
@
\TODO{DM: red is rather flashy - maybe use magenta instead?}
