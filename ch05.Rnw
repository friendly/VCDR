...
<<haireye-mos4, h=6, w=6, out.width='.49\\textwidth', cap='First step in constructing a mosaic display. Left: splitting the unit square according to frequencies of hair color; right: shading the tiles according to residuals from a model of equal marginal probabilities.', echo=FALSE, results='hide', crop=TRUE>>=
# one way
...
\item people with red hair and hazel eyes do not quite fit this pattern \TODO{Phil: I don't understand why this interpretation is here. The red/hazel combination was not significant or shaded, and seems to fit with the above trends. The only other marginally +ve residual was the red/green combo, which still generally fits with the first point. Remove?}
\end{itemize*}
...
\TODO{Phil: Add citation for Paul's grid graphics book?}
...
<<HE-fill, h=6, w=6, include=FALSE, out.width='.49\\textwidth', cap='Mosaic displays for the \texttt{haireye2} data, \TODO{Phil: haireye2 shows up as `exttthaireye2'...not exactly sure why, and this could be a re-occurring issue with texttt. FIXME} using custom colors to fill the tiles. Left: Marimekko chart, using colors to reflect the eye colors; right: Toeplitz-based colors, reflecting the diagonal strips in a square table.'>>=
fill_colors <- c("brown4", "#acba72", "green", "lightblue")
(fill_colors_mat <- t(matrix(rep(fill_colors, 4), ncol=4)))
mosaic(haireye2, gp = gpar(fill = fill_colors_mat, col = 0))
