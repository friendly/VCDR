...
\input{front/vtoc05}		% visual table of contents
...
\item can be used to illustrate the relations among variables that
are fitted by various \loglin models.
\end{itemize}

\ixon{mosaic display}
The basic ideas behind these graphical methods are explained for two-way tables in
\secref{sec:mosaic-twoway}; the \term{strucplot framework} on which these are
based is described in \secref{sec:mosaic-strucplot}. The graphical extension of
mosaic plots to three-way and large tables (\secref{sec:mosaic-threeway}) is quite direct.
...
Mosaic methods are further extended to all-pairwise plots in \secref{sec:mosmat} and 3D
plots in \secref{sec:3D}.
%\TODO{Complete this chapter overview}

\section{Two-way tables}\label{sec:mosaic-twoway}

\ixon{mosaic display!two-way tables}
\ixon{two-way table}
The mosaic display
...
% No longer need the code chunk -- froze the pdfcrop'd figures

% <<haireye-mos4, h=6, w=6, out.width='.49\\textwidth', cap='First step in constructing a mosaic display. Left: splitting the unit square according to frequencies of hair color; right: shading the tiles according to residuals from a model of equal marginal probabilities.', eval=FALSE, echo=FALSE, results='hide'>>=
% # one way
% mosaic(hair, pop=FALSE)
% labeling_cells(text = hair, gp_text = gpar(fontface=2, fontsize=18), clip=FALSE)(hair)
% 
% expected <- rep(sum(haireye) / 4, 4)
% names(expected) <- names(hair)
% expected
% residuals <- (hair - expected) / sqrt(expected)
% mosaic(hair, shade=TRUE, expected=expected, df=3, legend=F,
%   labeling=labeling_residuals, suppress=0, gp_text=gpar(fontface=2, fontsize=18))
% @
\begin{figure}[htb]
  \centering
...
\end{figure}
% \DONE{used pdfcrop and legend=FALSE}

\ix{marginal frequencies}
\ix{frequencies!marginal}
...
for red hair shows that redheads are much less common.  Further details
...
\ixoff{mosaic display!two-way tables}
\ixoff{two-way table}

\subsection{Shading levels}
\ix{mosaic display!shading}
\ix{graphical elements!shading}
...
\ixon{mosaic display!interpretation}
...
negative values indicate cells that occur less often than
under independence.
...
\item people with red hair and green eyes do not quite fit this pattern.
% \TODO{Phil: I don't understand why this interpretation is here. The red/hazel combination was not significant or shaded, and seems to fit with the above trends. The only other marginally +ve residual was the red/green combo, which still generally fits with the first point. Remove?}
\end{itemize*}

\ixoff{mosaic display!interpretation}

\section{The strucplot framework}\label{sec:mosaic-strucplot}
\ixon{strucplot framework}
...
\textbf{\emph{strucplot framework}} in the \Rpackage{vcd} provides a
...
\subsection{Components overview}\label{sec:strucplot_overview}
...
For the most part, you will directly use the convenience and related
...
\ixon{mosaic display!shading}
\ixon{graphical elements!shading}
...
support for both HSV (hue-saturation-value) and HCL (hue-chroma-luminance) colors, and
visualization of significance tests.
%(see Section \ref{sec:overview}).
%\DONE{This points to the need for a section, probably in
%  \chref{ch:intro}, on color spaces and color schemes for categorical
%  data graphics. Assigned DM as issue \#9. -- Added paragraph and ref in intro.}

\subsubsection{Specifying graphical parameters for strucplot displays}
Strucplot displays in \pkg{vcd} are built using the \basepkg{grid} graphics package
\citep{Murrell:2011}.
%\DONE{Phil: Add citation for Paul's grid graphics book?}
...
\begin{proglist}
  ...
  \item[\code{fontfamily}] 	The font family (serif, \textsf{sans},  \texttt{mono}, ...)
...
\footnote{Actually, the \code{fill\_colors} vector could be directly
...
@
\noindent Alternatively, we could have used the convenience function
\func{shading\_Marimekko} in \pkg{vcd}:
...
%% \end{figure}
\noindent Again, \pkg{vcd} offers a convenience function for this purposes,
\func{shading\_diagonal}:
...
\begin{itemize*}
  ...
  shading scheme, \code{shading\_Friendly} with HSV colors%
\footnote{\func{shading\_Friendly2} is a variant based on HCL colors.}
  of blue and red and
...
\item The right panel uses the \func{shading\_max} function, based on the ideas of
   ...
   visualizations.
   Instead of using the cutoffs 2 and 4, it employs the critical values, $M_\alpha$,
   for the maximum
...
half of the $\chi^2 (16) = 18.7$ for general association. The mosaic plot is
shown in \figref{fig:UKsoccer-mosaic}.
...
\end{Example}
\ixoff{mosaic display!shading}
\ixoff{strucplot framework}

\ixoff{residuals!shading}
\ixoff{graphical elements!shading}


\section{Three-way and larger tables}\label{sec:mosaic-threeway}

\ixon{three-way table!plotting}
\ixon{multi-way table!plotting}
...
with brown hair also looks suspicious.
\end{Example}
These and other hypotheses are best tested within the framework
of \term{loglinear model}s, allowing you to flexibly specify
various independence models for any number of variables,
and analyze them similarily to classical
ANOVA models. This general topic is discussed in detail
in Chapter \ref{ch:loglin}. For the present purposes, we give a short introduction in the following section.
\ixoff{three-way table!plotting}
\ixoff{multi-way table!plotting}

\subsection{A primer on \loglin models}\label{sec:loglinprimer}
\ixon{loglinear models}
\ixon{independence}
The essential idea behind \loglin models is that the multiplicative relationships
among expected frequencies under independence (shown as areas in sieve diagrams and mosaic plots)
become \emph{additive}
models when expressed as models for log frequency, and we briefly explain this connection here
for two-way tables.

To see this, consider two discrete variables, $A$ and $B$, with $n_{ij}$ observations in each cell $i,j$ of an $R \times C$
\ctab, and use $n_{i+} = \Sigma_j n_{ij}$ and $n_{+j} = \Sigma_i n_{ij}$ for the row and column marginal totals,
respectively. The total frequency is $n_{++} = \Sigma_{ij} n_{ij}$.
Analogously, we use $m_{ij}$ for the expected frequency under any model and also use a subscript $+$ to represent
summation over that dimension.
...
\begin{equation*}
 ...
 \period
\end{equation*}
This is usually re-expressed in an equivalent form in terms of model
parameters $\mu$, $\lambda_i^A$ and $\lambda_j^B$
\begin{equation} \label{eq:lmain0}
\log ( m_{ij} ) = \mu  +  \lambda_i^A +  \lambda_j^B \equiv [A] [B] %% \equiv \sim \mathtt{A + B}
  \period
\end{equation}
Model \eqref{eq:lmain0} asserts that the row and column variables
are independent because there is no term that depends on both $A$ and $B$.

\ix{two-way table}
In contrast, a model for a two-way table that allows an arbitrary
association between the variables is the \term{saturated model},
including an additional term, $\lambda_{ij}^{AB}$:
\begin{equation}\label{eq:lsat0}
\log ( m_{ij} ) = \mu  +  \lambda_i^A
+  \lambda_j^B  +  \lambda_{ij}^{AB} \equiv [AB] %% \equiv \sim \mathtt{A*B}
  \period
\end{equation}

\ix{analysis of variance}
\ix{interactions}
Except for the difference in notation, model \eqref{eq:lsat0} is formally the same
...
In contrast to ANOVA, the
``main effects,'' $\lambda_i^A$ and $\lambda_j^B$, are rarely
of interest---a typical log-linear analysis focuses only on the
interaction (association) terms.

Models such as \eqref{eq:lmain0} and \eqref{eq:lsat0} are
...
Thus, the saturated model, \eqref{eq:lsat0} contains $\lambda_{ij}^{AB}$,
...
notation that lists only the high-order terms: model \eqref{eq:lsat0}
is denoted $[A B]$, while model \eqref{eq:lmain0} is $[A] [B]$.

\ix{table form}
In \R, the most basic function for fitting \loglin models is
\func{loglin} in the \basepkg{stats} package.
...
to be fitted. For example, the independence model \eqref{eq:lmain0}
is specified as
<<ch5-loglm1, eval=FALSE>>=
loglin(mytable, margin = list(1, 2))
@
\noindent meaning that variables 1 and 2 are independent,
whereas the saturated model \eqref{eq:lsat0} would be specified as
...
With table variables \code{A} and \code{B}, the independence model can be
fit using \func{loglm} as
<<ch5-loglm3, eval=FALSE>>=
loglm(~ A + B, data = mytable)
@
\noindent and the saturated model in either of the following equivalent forms:
<<ch5-loglm4, eval=FALSE>>=
loglm(~ A + B + A : B, data = mytable)
loglm(~ A * B, data = mytable)
@
% \noindent or simply
% <<ch5-loglm5, eval=FALSE>>=
% loglm(~ A * B, data = mytable)
% @
\noindent In such model formulas, \code{A:B} indicates an interaction term $\lambda_{ij}^{AB}$,
while \code{A*B} is expanded to also include the terms \code{A + B}.
...
\code{haireye} two-way table, the independence of \var{Hair} and
\var{Eye} is equivalent to the model \LLM{Hair,Eye} and formulated in
\R using \func{loglm} as:
<<>>=
loglm(~ Hair + Eye, data = haireye)
@
\ix{statistical analyses!deviance tests}
\ix{deviance test statistics}
...
HE_S
@
\noindent giving a non-significant Pearson $\chi^2 (15) = 19.567, p=0.189$.
The residuals from this model could be retrieved using
\ix{residuals!Pearson}
\ix{Pearson residuals}
<<eval=FALSE>>=
residuals(HE_S, type = "pearson")
@
\noindent for further inspection. Mosaic plots can conveniently be used for this
...
is specified as \verb|expected = ~ Hair * Eye| \code{+ Sex},
giving \figref{fig:HEC-mos1}.
\ix{strucplot framework!labeling}
...
hair products.
\end{Example}
\ixoff{loglinear models}
\ixoff{independence}

\subsection{Fitting models}\label{sec:mosaic-fitting}
\ixon{loglinear models!fitting}
\ix{multi-way table!fitting}
\ix{three-way table!fitting}
...
For a three-way table, with variables $A$, $B$ and $C$, some of the hypothesized models that can be fit are
...
of the table that are fitted exactly, and which translate
directly into \R formulas used in \func{loglm} and
\code{mosaic(..., expected=)}.
%\DONE{Tweak the association diagrams here to use smaller circles, allowing longer connecting lines.}
...
\ix{independence!classes of}
\begin{description}

\ix{complete independence}
...
\begin{Example}[HEC3]{Hair color, eye color, and sex}
We continue with the analysis of the \data{HairEyeColor} data from
\exref{ex:HEC1} and \exref{ex:HEC2}.  \figref{fig:HEC-mos1} showed the fit of the
...
testing whether hair color and eye color are independent, given sex.
...
@
\end{Example}
\ixoff{loglinear models!fitting}
...
The main types of these models are illustrated in \tabref{tab:seqmodels} for 3-, 4-, and 5-way
tables, with variables A, B, ..., E.  In all cases, the natural model for the one-way
...
% \TODO{Phil: There seems to be a space after using the pkg command, so the period is off. Fix?}
...
In this section we have described a variety of models that can be fit
to higher-way tables, some relations among those models, and the aspects
of lack of fit that are revealed in the mosaic displays.
The following discussion illustrates the process of model fitting,
...
variables
(\citealp{Goodman:73}, \citealp[\S 7.2]{Fienberg:80}).  Suppose, for example,
...
recursive logit models that treat
\begin{seriate}
\item $B$ as a response to $A$,
\item $C$
as a response to $A$ and $B$ jointly, and 
\item $D$ as a response to $A$,
$B$ and $C$.
\end{seriate}

These are equivalent to the \loglin models that
...
These data were analyzed by \citet[\S 6.1.7]{Agresti:2013}
...
sample.  The mosaic for the model of \IX{joint independence},
...
Four-way mosaic plots for two models are shown in \figref{fig:presex3}.
...
\ixon{association!partial}

\ix{contingency table}
...
G_{A\perp B\given C}^2=\sum_k^K G_{A\perp B\given C(k)}^2 \period
\end{equation}
...
Data from a 1974 Danish study of 1,314 employees who had been laid off
...
gives the table below, illustrating the additive property of \GSQ 
and \chisq (\eqref{eq:partial1}).
\medskip
\begin{center}
\begin{tabular}{lrrr}
 \hline
...
\ix{mosaic display!three-way tables}
...
\begin{seriate}
 ...
 \item \code{education} level of respondent (elementary, secondary, high); and
...
shown in \figref{fig:punish-cond1-bis}.  \func{cotabplot} is very general, allowing
...
@
Both \figref{fig:punish-cond1-bis} and \figref{fig:punish-cond2} reveal that the
...
\func{cotabplot} of \figref{fig:punish-cond1-bis}, the marginal frequencies of
...
The \textbf{\emph{scatterplot matrix}}
%(\SSSGref{8.3.2})
...
into a single display, which allows detection of patterns that could
...
@
The mosaic matrix for these data, showing all two-way marginal relations, is shown
...
\ixon{mosaic display!generalized matrix}
\ixon{pairs plot}
We need not show only the marginal relation between
each pair of variables in a mosaic matrix.
\citet{Friendly:99:EMD} describes the extension of this idea
...
% <<berk-pairs3, h=8, w=8, out.width='.8\\textwidth', cap='Generalized mosaic matrix of the UCBAdmissions data. The off-diagonal plots fit models of conditional indpendence.', fig.pos='!htb'>>=
<<berk-pairs3, eval=FALSE>>=
pairs(UCBAdmissions, type = "conditional", space = 0.2)
@
\begin{figure}[htb]
  \centering
  \includegraphics[width=.8\textwidth]{ch05/fig/berk-pairs3}
  \caption{Generalized mosaic matrix of the UCBAdmissions data. The off-diagonal plots fit models of conditional indpendence.}
  \label{fig:berk-pairs3}
\end{figure}

Thus, the shading in the (1, 2) and (2, 1) panels shows the fit of the model
...
across departments.
%\TODO{This isn't quite right! MF: Restored earlier plot version!}
...
and then in a wider context by \citet{Emerson-etal:2013} and \citet{Friendly:2013:genpairs}.
...
\end{description*}
In \R some of these possibilities are provided in the \Rpackage{gpairs} (using \basepkg{grid} graphics
...
\ixoff{mosaic display!generalized matrix}
\ixoff{pairs plot}


\section{3D mosaics}\label{sec:3D}

\ixon{mosaic display!3D}
\ixon{interactive graphics}
\ixon{graphics!interactive}
\ixon{dynamic graphics}
\ixon{multi-way table}
...
which mitigates these problems by:
\begin{seriate}
...
\begin{figure}[!htb]
 \centering
 \includegraphics[width=0.6\textwidth]{ch05/fig/mos3d-bartlett.jpg}
 \caption{3D mosaic plot of the Bartlett data, according to the model of mutual independence.}\label{fig:mos3d-bartlett}
\end{figure}
% \TODO{Phil: I would recommend saving the rgl objects as a pdf file via 
% rgl.postscript('filename.pdf', 'pdf'). Should look a lot nicer than a pixelated png file}
...
% @
\ixoff{mosaic display!3D}
\ixoff{interactive graphics}
\ixoff{graphics!interactive}
\ixoff{dynamic graphics}
\ixoff{multi-way table}

\section{Visualizing the structure of \loglin models}\label{sec:mosaic-struc}

\ixon{loglinear models}
\ixon{contingency table}
...
for multiple regression or nonlinear regression with two predictors,
...
in turn, leads to a clearer understanding of the structure that appears
...
\ixon{mutual independence}

For example, to show the structure of a table that exactly fits the model of
...
the result in \figref{fig:struc-mos3}.  We use \code{gp=shading\_Friendly}
to explicitly indicate the zero residuals in the display,
\end{sloppypar}
% \TODO{@DM: FIXME-- The code was changed to use gp=shading_Friendly2, w/o mention, and the figure, 5.31, is not referred to in the text}
% DM: the original reference above was alreadey wrong in the submitted
% version - should be struc-mos3 instead of struc-mos2.
...
that these two variables are marginally independent.
...
\begin{figure}[!htb]
 \centering
 \includegraphics[width=0.6\textwidth]{ch05/fig/struct-mos3d1.jpg}
...
\end{figure}
\ixoff{mutual independence}

\subsection{Joint independence}
\ixon{joint independence}
