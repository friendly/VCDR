...
\input{front/vtoc04}		% visual table of contents
...
displays help us to visualize the pattern of association,
...
studied from data that has been
...
Some examples are also presented that involve a third, \term{stratifying variable},
...
The main emphasis, however, is on graphical methods that help
...
of $k$ such tables.
\boldital{Sieve diagrams}
\ix{sieve diagram}
(\secref{sec:twoway-sieve}) and \term{association plot}s
...
In order to make clear some of the distinctions that occur in
...
See \data{UCBAdmissions} (in package \basepkg{datasets}) for the complete data set.
...
flexibly implemented by function \func{fluctile} in the package \pkg{extracat}.}.
...
\ixon{two-way table!tests of association}
\ixon{tests of association}
...
\ixon{variable!response}
\ixon{response variables}
\ixon{variable!explanatory}
\ixon{explanatory variables}
...
\ixoff{variable!response}
\ixoff{response variables}
\ixoff{variable!explanatory}
\ixoff{explanatory variables}
\ixoff{two-way table!tests of association}
\ixoff{tests of association}

\subsection{2 by 2 tables: Odds and odds ratios}\label{sec:twoway-twobytwo}

The 2 by 2 \ctab of applicants to Berkeley graduate programs in \tabref{tab:berk22} may be regarded as an example of a
\term{cross-sectional study}.
\ix{table form}
\ix{data frame!multinomial}
\ix{cross-classification}
...
\end{equation*}
\ixon{independence}
\ixon{hypothesis!independence vs. homogeneity}
...
\ixoff{independence}
\ixoff{hypothesis!independence vs. homogeneity}
\ixoff{homogeneity}
...
\ctabs, the \textbf{\emph{odds ratio}} is particularly useful for
...
gives an equivalent measure that varies \emph{additively} around 0, called the
...
success probabilities for Group 1 and Group 2.  The \textbf{\emph{odds ratio}}, $\theta$,
...
a measure that is symmetric about 0.
...
% \TODO{PC: Perhaps mention the profiled-likelihood approach to obtain exact CI's for 
% smaller samples in a footnote? A good reference for that is Pawitan 2001, 'In all likelihood'}
...
\begin{equation}\label{eq:aselogtheta}
 ...
 =   \sqrt{ \sum_{i,j} n_{ij}^{-1}} \period
\end{equation}
\ix{odds ratio!confidence interval}
...
confint(LOR)
@ 
\ix{distributions!hypergeometric}
\ix{hypergeometric distribution}
\ix{statistical analyses!exact vs. asymptotic tests}
\begin{sloppypar}
...
\var{Improved}, an overall analysis (that ignores \var{Sex}) can be carried out
...
\ixon{sieve diagram}
\ixon{Cochran--Mantel--Haenszel tests!general association}
\ix{weights}
...
%@
\ixoff{sieve diagram}
\ixoff{Cochran--Mantel--Haenszel tests!general association}

\subsubsection{Linear association}

\ixon{Cochran--Mantel--Haenszel tests!linear association}
...
\ixoff{Cochran--Mantel--Haenszel tests!linear association}
\ixoff{ordinal variables}

\section{Stratified analysis}\label{sec:twoway-strat}

\ixon{categorical data!stratifying}
\ixon{variable!stratifying}
\ixon{stratified analysis}
\ixon{Cochran--Mantel--Haenszel tests!stratified analysis}
...
A \textbf{\emph{stratified analysis}} controls for the effects of one or more background variables.
...
\ix{statistical analyses!Cramer's V} 
\ix{Cramer's V}
...
\ixoff{categorical data!stratifying}
\ixoff{variable!stratifying}
\ixoff{stratified analysis}
\ixoff{Cochran--Mantel--Haenszel tests!stratified analysis}

\end{Example}

\subsection{Assessing homogeneity of association}\label{sec:twoway-homog}
\ixon{homogeneity of association}
...
\ix{statistical analyses!Woolf's test}
\ix{Woolf's test}
...
As will be discussed later (\secref{sec:mosaic-threeway})
\ix{loglinear models}
\ix{three-way table}
...
\LLM{AB,AC,BC}.
This notation (described in \secref{sec:loglinprimer} and \secref{sec:loglin-counts})
...
\ixoff{homogeneity of association}
...
\ixon{fourfold display!confidence rings}
...
\ixoff{fourfold display!confidence rings}
...
\ix{plotting!stratified displays}
...
This provides a clear standard that
also greatly facilitates controlled comparison.

As mentioned in the introduction, another principle is \term{visual impact}---we want the important
features
...
summary(lmod)
@
% \DONE{PC: The LOR DV doesn't take into account the precision wrt each odds ratio. 
% Adding a weights= argument to lm() containing the respective observed information 
% (w = diag(solve(vcov(lor_CM)))) would help correct this (obviously a glm would
% be better, but this is a reasonable approach when using lm). }
...
\ix{contingency tables!plotting}
...
would give a data display that implicitly compares observed
...
In World War II, all workers in the UK Royal Ordnance factories
...
(\citet[Table 33.5]{KendallStuart:61} and
...
the \textbf{\emph{strucplot framework}}, a general system for
visualizing $n$-way frequency tables in a hierarchical
way.  We describe this framework in more detail in
\secref{sec:mosaic-strucplot} in the context of mosaic
...
basically is a loss of information, but advantageous if the distribution of the conditioning variable(s) is
...
plots for this data. The following calls produce \figref{fig:berkeley-cotabsieve} and \figref{fig:berkeley-cotabsieve2},
with different conditioning and styles.
...
of \term{joint independence}, \LLM{Admit Gender,Dept}.
...
\ix{independence}
In the \textbf{\emph{sieve diagram}} the foreground (rectangles) shows expected
...
\textbf{\emph{observer agreement}} rather than mere association.
...
For an $R \times R$ table, two commonly used patterns of weights are those based on
...
\end{eqnarray*}
\ix{Fleiss-Cohen weights}
\ix{weights}
\ix{Cicchetti Allison weights}
...
\ixoff{agreement!observer agreement chart}
\ixoff{observer agreement chart}

\subsubsection{Partial agreement}
\ixon{agreement!partial}
...
\code{agreementplot} panel function to handle the \var{Patients} stratum:
...
%\TODO{PC: x-axis labels overlap. Change to angled labels? FIXME -- Not enough to worry about}

\end{Example}
\ixoff{agreement!observer bias}
\ixoff{observer agreement}
\ixoff{agreement}
...
@
%\DONE{PC: ggtern apparently doesn't handle legends very well; the geom_text() call makes the labels
%go screwy. Consider removing the legend somehow? (theme(..., legend.position='none') 
%seems to break for some reason, so might need to to this manually for now....)}
<<eval=FALSE>>=
library(ggtern)
DATA <- data.frame(
  A = c(40, 20, 10),
...
variable, and the percentage of this variable decreases
...
We examine the question of who survived and why in the sinking of the \emph{RMS Titanic} in \chref{ch:mosaic} (\exref{ex:titanic-ddecker}, \exref{ex:titanic-lor}, \labref{lab:mosaic-titanic}),
...
contains the data listed on page 38 of that report.%
...
<<cleanup04, include=FALSE, purl=FALSE>>=
