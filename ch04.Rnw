...
summary(lmod)
@
\TODO{PC: The LOR DV doesn't take into account the precision wrt each odds ratio. 
...
\TODO{PC: x-axis labels overlap. Change to angled labels? FIXME}
...
ggtern(data = DATA,
    ...
    guides(size = "none")
@
\TODO{PC: ggtern apparently doesn't handle legends very well; the geom_text() call makes the labels
