...
<<cmhdemo, h=6, w=6, out.width='.49\\textwidth', cap='Sieve diagrams for two patterns of association: Left: General association; right: Linear association'>>=
...
<<HE-sieve, h=6, w=6, out.width='.49\\textwidth', cap='Sieve diagrams for Hair Eye color data. Left: expected frequencies shown in cells as numbers and the number of boxes; right: observed frequencies shown in cells.', echo=FALSE>>=
...
<<VA-sieve2, h=6, w=6, out.width='.7\\textwidth', cap='Vision classification for 7477 women in Royal Ordnance factories'>>=
...
<<HE-assoc, h=6, w=6, out.width='.6\\textwidth', cap='Association plot for the two-way table of hair color by eye color.'>>=
haireye <- margin.table(HairEyeColor, 1:2)
...
<<HE-assoc2, h=6, w=6, out.width='.6\\textwidth', cap='Association plot for the three-way table of hair color by eye color by sex.'>>=
...
<<sexfun-agree, h=6, w=6, out.width='.48\\textwidth', cap="Agreement charts for husbands' and wives' sexual fun. Left: unweighted chart, showing only exact agreement; right: weighted chart, using weight $w_1 = 8/9$ for a one-step disagreement.">>=
...
<<MS-agree, h=6, w=6, out.width='0.49\\textwidth', cap='Weighted agreement charts for both patient samples in the MSPatients data. Departure of the middle rectangles from the diagonal indicates lack of marginal homogeneity.', echo=c(2,3)>>=
op <- par(mar=c(4,3,4,1)+.1)
...
<<tripdemo2, h=6, w=6, out.width='.6\\textwidth', cap='A trilinear plot showing three points, for variables A, B, C.', out.extra='trim=20 20 20 20,clip'>>=
library(ggtern)
DATA <- data.frame(
  A = c(40, 20, 10),
  B = c(30, 60, 10),
...
<<lifeboats1, h=6, w=6, out.width='.7\\textwidth', cap='Lifeboats on the \\emph{Titanic}, showing the composition of each boat.  Boats with more than 10\\% male passengers are labeled.', scap='Lifeboats on the Titanic', out.extra='clip'>>=
...
<<lifeboats2, h=5, w=9, out.width='.8\\textwidth', cap='Number of people loaded on lifeboats on the Titanic vs. time of launch, by side of boat. The plot annotations show the linear regression and loess smooth.'>>=
