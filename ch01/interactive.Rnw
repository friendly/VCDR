...
range from
\begin{itemize*}
\ix{graphical elements!controls}
...
The \Rpackage{animation} facilitates creating animated graphics  and movies in a
variety of formats.
...
\emph{The New York Times} published an interactive graphic,%
...
weighted
by the number of Electoral College votes. \emph{Source}: Mike Bostock \& Shan Carter, \emph{New York Times} interactive, November 2, 2012. Used by permission.}
