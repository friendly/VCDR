...
some solutions or hints for the various problems.  Answers are indicated
with the $\bigstar$ symbol, and with \textsf{text in this font}.
...
\definecolor{shadecolor}{rgb}{0.941, 0.941, 0.941}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{> }\hlkwd{mosaic}\hlstd{(criminal,} \hlkwc{shade}\hlstd{=}\hlnum{TRUE}\hlstd{,} \hlkwc{labeling}\hlstd{=labeling_residuals,} \hlkwc{suppress}\hlstd{=}\hlnum{0}\hlstd{)}
\hlstd{> }\hlkwd{mosaic}\hlstd{(criminal,} \hlkwc{gp}\hlstd{=shading_Friendly,} \hlkwc{labeling}\hlstd{=labeling_residuals,} \hlkwc{suppress}\hlstd{=}\hlnum{0}\hlstd{)}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.941, 0.941, 0.941}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{> }\hlkwd{mosaic}\hlstd{(aircrash.tab,} \hlkwc{shade}\hlstd{=}\hlnum{TRUE}\hlstd{,} \hlkwc{rot_labels}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{20}\hlstd{,}\hlnum{90}\hlstd{,}\hlnum{0}\hlstd{,}\hlnum{70}\hlstd{))}
\hlstd{> }\hlkwd{mosaic}\hlstd{(aircrash.tab,} \hlkwc{shade}\hlstd{=}\hlnum{TRUE}\hlstd{,} \hlkwc{alternate_labels}\hlstd{=}\hlnum{TRUE}\hlstd{)}
\end{alltt}
\end{kframe}
...
\hlstd{> }\hlkwd{mosaic}\hlstd{(aircrash.tab[roworder,],} \hlkwc{shade}\hlstd{=}\hlnum{TRUE}\hlstd{,} \hlkwc{rot_labels}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{20}\hlstd{,}\hlnum{90}\hlstd{,}\hlnum{0}\hlstd{,}\hlnum{70}\hlstd{))}
\hlstd{> }
...
\hlstd{> }\hlstd{aircrash.ca}
\end{alltt}
\begin{verbatim}

 Principal inertias (eigenvalues):
           1        2        3       4       
Value      0.123002 0.051548 0.01234 0.000562
Percentage 65.62%   27.5%    6.58%   0.3%    


 Rows:
         en route   landing  standing  take-off   unknown
Mass     0.357631  0.478360  0.009112  0.148064  0.006834
ChiDist  0.282474  0.324494  2.292345  0.612039  0.872559
Inertia  0.028536  0.050370  0.047880  0.055463  0.005203
Dim. 1  -0.646053  0.910702 -5.413140 -1.099071  1.091749
Dim. 2  -0.630689 -0.073348 -4.720748  2.090502 -0.859449


 Columns:
         criminal human error mechanical  unknown   weather
Mass     0.052392     0.47153   0.168565  0.11845  0.189066
ChiDist  1.122062     0.17033   0.598187  0.33281  0.426378
Inertia  0.065962     0.01368   0.060317  0.01312  0.034372
Dim. 1  -2.308247     0.44543  -1.488701 -0.40903  1.112281
Dim. 2  -3.396324     0.24831   1.252718 -0.10358 -0.730118
\end{verbatim}
\begin{alltt}
...
\end{alltt}
\begin{verbatim}
      1B       2B       3B        C       CF       DH       LF       OF        P       RF 
26.09995  0.95479  0.65037  0.95194 26.28438 17.44900 22.41938 21.38510 27.98363 23.57075 
      SS 
 0.81835 
\end{verbatim}
\begin{alltt}
...
\hlstd{> }\hlkwd{plot}\hlstd{(Dim2} \hlopt{~} \hlstd{Dim1,} \hlkwc{type}\hlstd{=}\hlstr{'n'}\hlstd{,} \hlkwc{data}\hlstd{=coords)}
...
\definecolor{shadecolor}{rgb}{0.941, 0.941, 0.941}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{> }\hlstd{res} \hlkwb{<-} \hlkwd{plot}\hlstd{(accident.mca,} \hlkwc{labels}\hlstd{=}\hlnum{0}\hlstd{,} \hlkwc{pch}\hlstd{=}\hlstr{'.'}\hlstd{,} \hlkwc{cex.lab}\hlstd{=}\hlnum{1.2}\hlstd{)} \hlcom{#makes plot with no labels or points}
\hlstd{> }\hlstd{coords} \hlkwb{<-} \hlkwd{data.frame}\hlstd{(res}\hlopt{$}\hlstd{cols, accident.mca}\hlopt{$}\hlstd{factors)}\hlcom{#to extract factor names & levels }
...
\hlstd{> }\hlstd{donner.mod4a} \hlkwb{<-} \hlkwd{glm}\hlstd{(survived} \hlopt{~} \hlkwd{poly}\hlstd{(age,}\hlnum{2}\hlstd{)} \hlopt{*} \hlstd{sex} \hlopt{+} \hlstd{family.size,} \hlkwc{data}\hlstd{=Donner,} \hlkwc{family}\hlstd{=binomial)}
...
\centerline{\includegraphics[width=.75\textwidth]{soln/fig/ex7_4b-1} }
...
\centerline{\includegraphics[width=.75\textwidth]{soln/fig/ex7_4c-1} }
...
44         Pre    1      B2      no    mid      few     healthy     2
84         Pre    2      C4      no   high      few        dead     1
98         Pre    2      B1    full    mid      few     healthy     2
133        Pre    3      B1      no    mid      few        dead     2
159 Incubation    4      C2 partial    low      few        dead     1
176 Incubation    4      C4    full   high      few        dead     1
243 Incubation    6      C2    full    mid      few        dead     1
248 Incubation    6      C3      no   high     high        dead     1
