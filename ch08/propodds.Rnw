...
The \textbf{\emph{proportional odds model}} (PO) \citep{Mccullagh:1980} proposes a simple
...
% \DONE{DM: Fig. 8.1 shows $\Pr (Y > j \given \vec{x})$, but in the
%   text, we use $\Pr (Y \le j \given \vec{x})$.}
\begin{figure}
 \centering
 \includegraphics[width=.49\textwidth]{ch08/fig/podds2}
 \includegraphics[width=.49\textwidth]{ch08/fig/podds1}
...
are constant and their difference is $(\alpha_j - \alpha_{j\prime})$,
so the odds have a constant ratio $\exp (\alpha_j - \alpha_{j\prime})
= \exp (\alpha_j) / \exp (\alpha_{j\prime})$, or are proportional.
Similarly,
...
\ixon{proportional odds model!latent variable interpretation}
\ix{latent variable}
\ix{binary}
...
\begin{equation}\label{eq:latent}
 \xi_i = \alpha + \vec{x}_i \trans \beta + \epsilon_i
        = \alpha + \beta_1 x_{i1} + \cdots + \beta_p x_{ip} + \epsilon_i \period
\end{equation}
\ix{scaling}
...
Similarly, the probit model arose from dose-response studies in toxicology \citep{Bliss:1934,Finney:1947}
...
development of \IX{factor analysis} \citep{Bentler:1980},
%\DONE{citation?}
...
%\DONE{Add a schematic graph here: Fig 14.10 from Fox:2008, which he adapted from Agresti:1990, Fig 9.2}

\begin{figure}
 \centering
 \includegraphics[width=.9\textwidth]{ch08/fig/latent}
...
\ixoff{proportional odds model!latent variable interpretation}


\subsection{Fitting the proportional odds model}

\ixon{proportional odds model!fitting}
...
In the \data{Arthritis} data, the response \var{Improved} was set up this
...
The \pkg{car}::\func{Anova} method gives the appropriate tests.
...
\ixoff{proportional odds model!fitting}

\subsection{Testing the proportional odds assumption}

\ixon{proportional odds model!assumptions}
...
The LR test can be also calculated ``manually'' as shown below using the difference
...
\ixoff{proportional odds model!assumptions}

\subsection{Graphical assessment of proportional odds}

\ixon{proportional odds model!plotting}
...
Harrell's work \emph{Regression Modeling Strategies} (\citeyear[Chapters 13--14]{Harrell:2001})
...
% \DONE{DM: sth. wrong with the numbering. Either use subsubsection
%   for the subsequent two topics, or move this introduction
...
different colors to the lines for the response categories, as seen in \figref{fig:arth-polr1}.
Here, \func{facet\_grid}
...
direct.label(gg)
@
Although we now have three response curves in each panel in \figref{fig:arth-polr1}, this plot is relatively easy to understand:
\begin{seriate}
...
\ixon{effect plot!proportional odds model}
\ixon{proportional odds model!effect plot}
...
@
\ixoff{effect plot!proportional odds model}
\ixoff{proportional odds model!effect plot}

\ixoff{proportional odds model!plotting}
