...
\TODO{DM: Fig. 8.1 shows $\Pr (Y > j \given \vec{x})$, but in the
...
Harrell's work \emph{Regression Modeling Strategies} (\citeyear[Ch. 13--14]{Harrell:2001})
...
\TODO{DM: sth. wrong with the numbering. Either use subsubsection
  for the subsequent two topics, or move this introduction
  to the beginning of the next one.}

\subsection{Full-model plots}\label{sec:po-fullplots}
...
\subsection{Effect plots}\label{sec:po-effplots}
