...
which maximimally account for their association.
...
More importantly, in some reasearch areas in the social sciences
...
<<mental-glm4>>=
vcdExtra::Summarise(indep, linlin, roweff, coleff)
@
...
This model has a close connection with corrrespondence analysis
...
In addition to the normalization contraints for the RC(1) model, 
parameters in an RC(M) model must satisfy the additional contraints
...
<<mental-gnm2>>=
vcdExtra::Summarise(indep, linlin, roweff, coleff, RC1, RC2)
@
The substantitive difference between the $L \times L$ model and the RC(1)
...
the the mental health data.  Error bars show $\pm 1$ standard error.}
...
convience functions that do the heavy lifting.  
...
\begin{description}
  ...
  caled RC(M)-L models by \cite{Wong:2010}.
