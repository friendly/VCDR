...
We plot these as shown below, using \func{matplot}, which is convienient for plotting
multiple columns against a given horizontal varible, \code{age} here.%
\footnote{
...
vcdExtra::Summarise(cm.glm0, cm.glm1)
@
...
it is apparent that both breathlessnes and wheeze increase with age,
...
vcdExtra::Summarise(cm.glm1, cm.glm2)
@
...
vcdExtra::Summarise(cm.glm0, cm.glm1, cm.glm2, cm.glm3)
@
...
\begin{seriate}
  \item It doesn't easily allow us to represent and test the substantitively interesting
  ...
  the coefficient for \code{ageOR} = \Sexpr{coef(cm.glm3)[["ageOR"]]} in a substantitively
