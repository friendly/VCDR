...
A substantitively more interesting hypothesis is whether the table
...
Another hypothesis of substantitive interest for square tables,
...
<<vision-glm4>>=
vcdExtra::Summarise(indep, quasi, symm, qsymm)
@
...
text(4, as.matrix(lor.hauser)[4,], rownames(lor.hauser), 
     pos=4, col=1:4, xpd=TRUE, cex=1.2)
...
vcdExtra::Summarise(hauser.indep, hauser.quasi)
@
...
vcdExtra::Summarise(hauser.qsymm)
@
...
vcdExtra::Summarise(hauser.topo)
@
...
\code{hauser.topo}.
<<hauser-summary>>=
vcdExtra::Summarise(hauser.indep, hauser.quasi, hauser.qsymm, hauser.topo)
@
...
% for both row and column variabales, 
...
applies directly to square tabels, and, for square tables, can also be
ammended to include a diagonals term, \func{Diag}, giving a model
...
<<hauser2-ord2>>=
vcdExtra::Summarise(hauser.indep, hauser.UA, hauser.roweff, hauser.RC)
@
...
vcdExtra::Summarise(hauser.CR, hauser.CRdiag)
@
...
sumry <- vcdExtra::summarise(modlist)
...
in terms of BIC.  Similar plots for AIC and $\GSQ/df$ show that the the model of
