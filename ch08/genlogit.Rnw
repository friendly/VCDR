...
\begin{eqnarray}\label{eq:glogit1}
  ...
  \beta _{kj} \,  x_{ip} \quad
   j=1, \dots , m-1 \nonumber \\
  & = & {\vec{x}_i} \trans \vec{\beta}_j \period
\end{eqnarray}
...
\begin{eqnarray*}
 ...
 \frac{ \exp ( {\vec{x}_i} \trans \vec{\beta}_j ) }
      ...
      \quad\quad j=1, 2, \dots m-1 \comma
 \\
 \pi_{im} & = & 1 - \sum_{i=1}^{m-1} \pi_{ij} \quad\quad\mbox{for } Y=m \period
\end{eqnarray*}
...
\begin{eqnarray*}%\label{eq:glogitab}
  L_{ab} & = & \log \frac{\pi_{ia}/\pi_{im}}{\pi_{ib}/\pi_{im}} \\
         ...
         & = & (\beta_{0a}-\beta_{0b}) + (\beta_{1a}-\beta_{1b}) x_{i1}
            + (\beta_{2a}-\beta_{2b}) x_{i2} \period
\end{eqnarray*}
...
\ixon{generalized logit model!plotting}
\ixon{multinomial logit model!plotting}
...
\ixoff{generalized logit model!plotting}
\ixoff{multinomial logit model!plotting}
