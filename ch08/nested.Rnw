...
The method of \textbf{\emph{nested dichotomies}}
provides another simple way to analyze a polytomous response in the
...
\ix{statistical analyses!Wald test}
...
\begin{eqnarray*}
  ...
  \vdots & = & \vdots \\
  L_{m-1} & = & \alpha_{m-1} + \vec{x} \trans \vec{\beta}_{m-1} \period 
\end{eqnarray*}
...
\ixon{nested dichotomies!plotting}
To plot these fitted values, we will again create a conditional
plot using \pkg{ggplot2}. Since this requires having all probabilities
in one column, together with an additional grouping variable
identifying the working status, we need to reshape \code{fit} from
``wide'' to ``long'' format, yet again using the \func{melt} from the \Rpackage{reshape2}:
<<wlf-reshape>>=
fit2 <- melt(fit,
...
\noindent The lines below give the plot shown in \figref{fig:wlf-fitted-prob}:
%\DONE{DM: this code is quite complicated. Why not using melt + ggplot2? -- MF: Pls fix!}
...
@
\noindent (Note the extension of the axes to avoid label clipping.)

We can see that the decision not to work outside the home
...
\ixoff{nested dichotomies!plotting}
\end{Example}
