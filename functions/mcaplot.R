...
#' @param pch  Vector of symbols for the category levels, one for each factor
...
mcaplot <- function(obj, map="symmetric", dim=1:2, 
  ...
  ylab      = "_auto_",
	...
	if(length(lines))
		multilines(coords[, dim], group=coords$factor, which=lines, col=cols, lwd=lwd)
