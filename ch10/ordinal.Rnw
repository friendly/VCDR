...
that maximally account for their association.
...
In these cases \loglin and logit models that use the ordered nature of the factors
offer several advantages:

\begin{itemize}
\item Because they are more focused, tests that use the ordinal
...
factors may be proposed that are intermediate between the independence
...
and debates, giving more precise tests of hypotheses than are
...
fit a \term{row plus column effects model} (R+C), which assigns specified
...
\ixon{log-multiplicative models}
...
\begin{eqnarray}
\sum_i \alpha_i \pi_{i+} & = & \sum_j \beta_j \pi_{+j} = 0 \comma \label{eq:RC-constraints} \\
\sum_i \alpha_i^2 \pi_{i+} & = & \sum_j \beta_j^2 \pi_{+j} = 1 \period \nonumber
\end{eqnarray}
...
of
\citet{Firth:2003} and also \citet{FirthMenezes:2004}. The details are explained in
\help{getContrasts} and in \code{vignette("gnmOverview")}, which comes with
the \Rpackage{gnm}.
...
\ixon{log-multiplicative models!plotting}
...
\caption{Dotchart of the scaled category scores for the RC(1) model fit to
...
\end{Example}
\ixoff{log-multiplicative models!plotting}
\ixoff{log-multiplicative models}
