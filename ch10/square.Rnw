...
The latter topic has been important in sociological and geographic
research
...
\ix{independence!in square tables}
...
that fits those frequencies perfectly.
...
when they differ, visual acuity in the two eyes is most likely to differ by
only one eye grade.
...
and Farm.  These data were also analyzed by \citet[\C 4]{PowersXie:2008}.

\ix{structable objects}
<<hauser1>>=
...
Among the features here, you can see that there is a tendency for the odds ratio
...
or \term{levels model}s, which are implemented in the \func{Topo} function.
...
levels <- matrix(c(
  ...
  5, 5, byrow = TRUE)
@
This model is fit using \func{Topo} as shown below. It also provides a huge
...
Both AIC and BIC prefer the quasi-symmetry model, \code{hauser.quasi}.
...
% \code{hauser.topo}. 
%{DM: Not true, BIC also prefers qsymm!}
<<hauser-summary>>=
...
\ixon{square tables!ordinal variables}
...
\ixoff{square tables!ordinal variables}
