...
Here, we consider models that examine the changes in prevalence of
...
The \data{CoalMiners} data is a $2 \times 2 \times 9$ table.  For convenience
...
coalminers
@
\func{vglm} takes the $2 \times 2$ response frequencies as a 4-column matrix on the left-hand
side of the model formula.  However, denoting the responses of
...
\begin{figure}[!b]
\centering
\includegraphics[width=.49\textwidth]{ch10/fig/cm-vglm1-prob}
\includegraphics[width=.49\textwidth]{ch10/fig/cm-vglm1-logit}
...
model indicates some slight tendency for the prevalence of breathlessness and wheeze to level off
slightly with age, particularly the former.
\begin{figure}[!htb]
\centering
\includegraphics[width=.8\textwidth]{ch10/fig/cm-vglm2-blogit}
