...
\begin{figure}[!htb]
\centering
\includegraphics[width=.75\textwidth]{ch10/fig/tox-fourfold-crop}
...
the function \func{loddsratio} in \pkg{vcd} calculates these for each $2 \times 2$
...
@
The \func{plot} method for the resulting \class{logoddsratio} object 
treats the conditioning variables in the formula argument as strata,
and plots the log odds ratios for the first such variable on the
horizontal axis with curves for the subsequent strata variables.
The lines below produce \figref{fig:tox-LOR}.
...
saturated model, \LLM{CSHU}, which fits perfectly, but with the data transformed
...
that allow the odds ratio to vary, first with smoking ($[SHU]$), then
...
As we did earlier, we first reshape the \data{Toxaemia} to wide format,
...
\begin{figure}
\centering
\includegraphics[width=.49\textwidth]{ch10/fig/tox-glm-logits1}
\includegraphics[width=.49\textwidth]{ch10/fig/tox-glm-logits2}
\caption{Observed (points) and fitted (lines) logits for the \data{Toxaemia} data under Model 2.}
...
\begin{figure}
\centering
\includegraphics[width=.49\textwidth]{ch10/fig/tox-glm-logits3}
\caption{Observed (points) and fitted (lines) log odds ratios for the \data{Toxaemia} data under Model 2.}
...
the log odds for hypertension depend mainly on smoking
