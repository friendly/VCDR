...
@
It is easy to see that survival among women was greater than for men,
...
survival and age is nonlinear is to re-do this plot, but now allow a quadratic
...
as shown in \figref{fig:donner1-cond3} may tell only part of the story,
...
\begin{itemize}
  \item Is the relationship different for men and women?  This is, is it necessary to allow for an interaction of age with sex, or separate fitted curves for men and women?
...
From a modeling perspective, there is a wide variety of approaches for testing for nonlinear
...
A simple way to test for nonlinearity is a likelihood ratio test comparing the
...
which fits the relationship with $x$ in terms of a set of piecewise
...
at the median, or 50$^{th}$ percentile; with $k=3$, the knots would be placed at the
...
\begin{seriate}
 ...
 \item whether the effect is linear on the logit scale or nonlinear (quadratic, here).
\end{seriate}
...
Next, we fit nonlinear models, representing the linear and nonlinear
...
in a table, together with their row and column differences.
...
nonlinearity here.  An alternative test of the same complexity
