...
logits were shown in \figref{fig:icu1-binreg-plot}.
...
in his having been unconscious on admission for an elective procedure, and
...
young, did not have cancer, and were conscious at admission.
...
An interesting alternative to individual index plots is a scatterplot matrix (\figref{fig:icu2-dbscatmat}) that shows the pairwise
changes in the regression coefficients for the various predictors.  Here we use
\func{scatterplotMatrix} from \pkg{car}, which offers features for additional plot
annotations, including identifying the most unusual points in each pairwise plot.
In each off-diagonal panel, a 95\% data ellipse and linear regression line help to
...
@
As \figref{fig:icu2-dbscatmat} illustrates, the \emph{joint} effect of observations on \emph{pairs} of coefficients
is more complex than is apparent from the univariate views that appear in the plots along the diagonal.
The DFBETAs for \var{cancer}, \var{admit}, and \var{uncons} are all extremely peaked, yet the pairwise plots
show considerable structure. The points identified would be worthy of further study.
