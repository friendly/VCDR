...
In this example we briefly examine some aspects of logistic regression
...
shown above and eliminate those predictors that fail significance at some
...
\item the strong likelihood that chance features of the data or outliers influence the result, and
...
three steps, which also removed \var{ph}, \var{pco}, and \var{systolic}.
<<icu1-coef>>=
coeftest(icu.step2)
@
...
simpler to interpret. We retain these both as ``candidate models'' to be explored further,
but for ease in this example, we do so using the smaller model, \code{icu.step2}.

Another important step is to check for nonlinearity of quantitative predictors such
as \var{age} and interactions among the predictors.  This is easy to do using
\func{update} and \func{anova} as shown below.  First, allow a nonlinear term
...
\figref{fig:icu-nomogram}, and is produced using \func{nomogram} in the \Rpackage{rms}
as follows.  This only works with models fit using \func{lrm}, so we have to
refit this model.
