...
for one specific model, the model \code{donner.mod3}, which included a 
...
% It is useful to see the entire collection because, by default, only
% four are plotted (\code{which = c(1:3, 5)}) and this selection 
...
<<donner2-inflmeasures2, size="footnotesize", R.options=list(width=95)>>=
summary(infl)
@

The simplest overview of adequacy of a fitted model is provided by
the \func{plot} method for a \class{glm} (or \class{lm}) object, which
can produce up to six different diagnostic plots. Among them, 
we consider the residual-leverage graph (number 5) as
being the most useful for assessing influential observations, plotting residuals against leverages. An
extended version is produced by the 
function \func{influencePlot} in the \Rpackage{car}, which
additionally uses the size (area)
