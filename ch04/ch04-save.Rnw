...
data (\tabref{tab:hairdat}), calculated as follows:
<<HE-expected>>=
...
sieve(expected, shade=TRUE, sievetype="expected", pop=FALSE, 
      main="Expected frequencies")
labeling_cells(text = round(expected,1), 
               gp_text = gpar(fontface = 2))(expected)
sieve(haireye, shade=TRUE, pop=FALSE, 
      main="Observed frequencies")
labeling_cells(text = haireye, 
               gp_text = gpar(fontface = 2))(haireye)
@
...
<<ms1>>=
MSPatients[,,"Winnipeg"]
sum(MSPatients[,,"Winnipeg"])
@
New Orleans patients:
<<ms2>>=
MSPatients[,,"New Orleans"]
sum(MSPatients[,,"New Orleans"])
@
...
\TODO{Figure out why these plots are deformed and clipped. Complete this section.}
\end{Example}
...
use them in detailed analyses of spatial and temporal changes in
