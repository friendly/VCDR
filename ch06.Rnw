...
<<ca-suicide-sup, echo=2:3, h=3, w=6, out.width='.9\\textwidth', cap='2D CA solution for the [Age] [Method] marginal table. Category points for Sex are shown as supplementary points.', scap='2D CA solution for the Age Method marginal table.'>>=
op <- par(cex=1.3, mar=c(4,4,1,1)+.1)
...
($\mat{\Sigma}$)
and there is some justification for regardin the Burt matrix as the
...
<<presex-mca-plot, h=6, w=6, out.width='.8\\textwidth', cap='MCA plot of the Burt matrix for the PreSex data. The category points are joined separately by lines for the factor variables.', fig.pos='!htb', out.extra='trim=0 10 20 60'>>=
