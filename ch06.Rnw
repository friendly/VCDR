...
Correspondence analysis (CA) is an exploratory technique that displays
...
dimensions that account for the greatest proportion of the
\(\chi^2\) for association between the row and column categories,
just as principal components account for maximum variance
of quantitative variables.  But CA does more---the scores provide a quantification of the categories,
...
or \citet{Gifi:81} and \citet{Lebart-etal:84}
...
as points in
the two (or three) dimensions that account for the greatest
...
\begin{eqnarray}
%
\mat{F} & = & \mat{D}_r^{-1/2} \mat{U} \mat{D}_{\lambda}^2 \quad\mbox{scaled so that} \quad \mat{F}\trans \mat{D}_r \mat{F} = \mat{D}_{\lambda}^2  \comma \label{eq:pcoord1} \\
\mat{G} & = & \mat{D}_c^{-1/2} \mat{V} \mat{D}_{\lambda}^2 \quad\mbox{scaled so that} \quad \mat{G}\trans \mat{D}_c \mat{G} = \mat{D}_{\lambda}^2 \period \label{eq:pcoord2}
\end{eqnarray}
...
\begin{eqnarray}
%\label{}
\mat{\Phi} & = & \mat{D}_r^{-1} \mat{U}  \quad\mbox{scaled so that} \quad \mat{\Phi}\trans \mat{D}_r \mat{\Phi} = \mat{I} \comma \label{eq:scoord1} \\
\mat{\Gamma} & = & \mat{D}_c^{-1} \mat{V} \quad\mbox{scaled so that} \quad \mat{\Gamma}\trans \mat{D}_c \mat{\Gamma} = \mat{I} \period \label{eq:scoord2}
\end{eqnarray}
...
\CA\ solutions that are useful in interpretation.
...
\begin{description*}
  ...
  the singular values.  
  There is also an \func{mca} function for
...
example, \pkg{MASS}::\func{corresp} handles matrices, data frames, and
...
form the columns.  By default, \func{ca} produces a two-dimensional
...
The data set \data{RepVict} in the \Rpackage{vcd} gives an $8 \times 8$ table
...
\ix{graphical elements!labeling}
...
profile, with Rape and Pickpocket as exceptions.

In fact, if the table was symmetric, the row and column points in \figref{fig:ca-victims-plot} would be identical,
...
\subsection{Correspondence analysis and mosaic displays}

\ixon{mosaic display!vs. correspondence analysis}
\ixon{correspondence analysis!vs. mosaic displays}
\ix{Pearson residual}
\ix{residuals!Pearson}
...
The main association here concerns how viewership across
days of the week varies by TV network, so we first collapse the \data{TV} data
...
slot.  Could the overall popularity of NBC on Thursday have been due to \emph{Friends} or \emph{Seinfeld}?
...
\ixoff{mosaic display!vs. correspondence analysis}
\ixoff{correspondence analysis!vs. mosaic displays}
...
\ca that analyzes simultaneously all possible two-way tables
...
\tabref{tab:stacking}.
In this approach, only the associations in separate $[ \, ]$ terms are analyzed
...
The original \(2 \times  17 \times  9\) table contains 17 age groups from 10 to 90 in
...
\ix{mosaic display}
\ix{overplotting}
\ix{graphical elements!overplotting}
To construct the mosaic display for the same model analyzed by \ca, we use the
...
To illustrate, the code below lists only the \code{age}
...
\ixoff{correspondence analysis!supplementary variables}

\section{Multiple correspondence analysis}\label{sec:mca}

\ixon{multiple correspondence analysis}
...
we illustrate the analysis of the indicator matrix below for the hair color--eye color 
data using \func{ca}, rather than the function \func{mjca},
...
\ixoff{multiple correspondence analysis}
...
in this equation. This calculation is shown below.
...
using the \code{Burt} matrix
produces the same display of the category points for
...
\ixon{multiple correspondence analysis!multivariate}
\ixon{multivariate data}
\ixon{indicator matrix}
...
($\mat{\Sigma}$)
and there is some justification for regarding the Burt matrix as the
...
\ixoff{multiple correspondence analysis!multivariate}
\ixoff{multivariate data}
\ixoff{indicator matrix}

\subsubsection{Inertia decomposition}

The transition from simple CA to MCA is straightforward in terms of
...
the \func{plot} function. \vspace{-20 pt}
...
Class and Age effects.  Consider those points that differ from the
origin most similarly (in distance and direction) to the point for Survived (``Yes'');
this gives the interpretation that survival was associated with being female
...
seen through the \emph{reconstitution formula}, giving the decomposition of the
...
\begin{equation}\label{eq:reconstitution1}
  p_{ij} = r_i c_j \left(1 + \sum_{m=1}^M \sqrt{\lambda_m} \phi_{im} \gamma_{jm} \right) \comma
\end{equation}
or, in matrix terms,
\begin{equation}\label{eq:reconstitution2}
  \mat{P} = \mat{D}_r (\vec{1} \vec{1}\trans + \mat{\Phi} \mat{D}_\lambda^{1/2} \Gamma\trans) \mat{D}_c \period
\end{equation}
...
\ixoff{singular value decomposition}
...
Two other types of asymmetric ``maps'' are also defined with different scalings
...
represents the methods as calibrated axis lines, designed to be read as scales
...
of the biplot display.  See \citet[Ch. 2]{Gower-etal:2011} for all the details.
...
\iff \mat{Y} \approx \vec{a} \vec{1}\trans + \vec{1} \vec{b}\trans \comma
\end{equation*}
...
be zero:
\begin{equation*}
A \perp B \iff \log \theta_{i i^{\prime}, j j^{\prime}} = \log \left( \frac{n_{ij} n_{i^{\prime} j^{\prime}}} {n_{i^{\prime} j}  n_{ij^{\prime}}} \right) = 0 \period
\end{equation*}
...
= ( \vec{a}_i - \vec{a}_{i^{\prime}} )\trans ( \vec{b}_i - \vec{b}_{i^{\prime}} ) \period
\end{equation*}
...
dimnames(UKSoccer) <- list(Home = paste0("H", 0:4),
                           Away = paste0("A", 0:4))
@
Basic biplots in \R are provided by \func{biplot}, which works mainly with
...
<<cleanup06, include=FALSE, purl=FALSE>>=
