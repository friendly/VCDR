#' ---
#' title: "Toxaemia data: Effect plots"
...
plot(allEffects(tox.urea, confidence.level=0.68),
  ...
  lwd=3, 
  multiline=TRUE,
  key.args=list(x=0.7, y=0.2, cex=1.2, columns=1)
  )
...
plot(Effect(c("class", "smoke"), tox.hyper1),
  ...
  lwd=3, 
  multiline=TRUE,
  key.args=list(x=0.05, y=0.15, cex=1.2, columns=1)
  )
...
plot(Effect(c("class", "smoke"), tox.urea1),
  ...
  lwd=3, 
  multiline=TRUE,
  key.args=list(x=0.75, y=0.15, cex=1.2, columns=1)
  )
